...
SET
    ...
    provider_type = COALESCE(
        sqlc.narg ('provider_type'),
        provider_type
    ),
    issuer_url = COALESCE(
        sqlc.narg ('issuer_url'),
        issuer_url
    ),
    client_id = COALESCE(
        sqlc.narg ('client_id'),
    ...
    client_secret_encrypted = COALESCE(
        sqlc.narg ('client_secret_encrypted'),
        client_secret_encrypted
    ),
    authorization_endpoint = COALESCE(
        sqlc.narg ('authorization_endpoint'),
        authorization_endpoint
    ),
    token_endpoint = COALESCE(
        sqlc.narg ('token_endpoint'),
        token_endpoint
    ),
    userinfo_endpoint = COALESCE(
        sqlc.narg ('userinfo_endpoint'),
        userinfo_endpoint
    ),
    jwks_uri = COALESCE(
        sqlc.narg ('jwks_uri'),
    ...
    end_session_endpoint = COALESCE(
        sqlc.narg ('end_session_endpoint'),
        end_session_endpoint
    ),
    scopes = COALESCE(sqlc.narg ('scopes'), scopes),
    claim_mappings = COALESCE(
        sqlc.narg ('claim_mappings'),
        claim_mappings
    ),
    role_mappings = COALESCE(
        sqlc.narg ('role_mappings'),
        role_mappings
    ),
    auto_create_users = COALESCE(
        sqlc.narg ('auto_create_users'),
        auto_create_users
    ),
    update_user_info = COALESCE(
        sqlc.narg ('update_user_info'),
        update_user_info
    ),
    allow_linking = COALESCE(
        sqlc.narg ('allow_linking'),
        allow_linking
    ),
    is_enabled = COALESCE(
        sqlc.narg ('is_enabled'),
        is_enabled
    ),
    is_default = COALESCE(
        sqlc.narg ('is_default'),
        is_default
    )
WHERE
    id = $1 RETURNING *;
...
SET
    ...
    access_token_encrypted = COALESCE(
        sqlc.narg ('access_token_encrypted'),
        access_token_encrypted
    ),
    refresh_token_encrypted = COALESCE(
        sqlc.narg ('refresh_token_encrypted'),
        refresh_token_encrypted
    ),
    token_expires_at = COALESCE(
        sqlc.narg ('token_expires_at'),
        token_expires_at
    ),
