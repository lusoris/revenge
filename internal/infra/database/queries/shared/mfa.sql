...
SELECT * FROM public.webauthn_credentials WHERE id = $1;

-- name: GetWebAuthnCredentialByCredentialID :one
-- Get a WebAuthn credential by its credential ID
SELECT * FROM public.webauthn_credentials WHERE credential_id = $1;
...
INSERT INTO
    public.mfa_backup_codes (user_id, code_hash)
VALUES ($1, $2);
...
SELECT * FROM public.user_mfa_settings WHERE user_id = $1;
