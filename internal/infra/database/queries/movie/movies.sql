...
SET
    ...
    radarr_id = COALESCE(sqlc.narg('radarr_id'), radarr_id),
    metadata_updated_at = COALESCE(sqlc.narg('metadata_updated_at'), metadata_updated_at)
...
INSERT INTO public.movie_files (
    movie_id,
    file_path,
    file_size,
    resolution,
...
SET
    ...
    radarr_file_id = COALESCE(sqlc.narg('radarr_file_id'), radarr_file_id)
WHERE id = sqlc.arg('id') AND deleted_at IS NULL
RETURNING *;
...
INSERT INTO public.movie_credits (
    movie_id,
    tmdb_person_id,
    name,
    credit_type,
...
SELECT * FROM public.movie_credits
WHERE movie_id = $1
...
SET collection_order = EXCLUDED.collection_order;
...
SELECT c.* FROM public.movie_collections c
...
ON CONFLICT (movie_id, tmdb_genre_id) DO NOTHING;

-- name: ListMovieGenres :many
SELECT * FROM public.movie_genres
WHERE movie_id = $1
ORDER BY name ASC;
...
INSERT INTO public.movie_watched (
    user_id,
    movie_id,
    progress_seconds,
    duration_seconds,
...
SET
    ...
    watch_count = CASE
        WHEN EXCLUDED.is_completed AND NOT movie_watched.is_completed
        THEN movie_watched.watch_count + 1
...
SELECT * FROM public.movie_watched
