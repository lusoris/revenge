...
UPDATE movie.movie_genres SET slug = REGEXP_REPLACE(slug, '-+$', '');
...
ALTER TABLE movie.movie_genres
ADD CONSTRAINT uq_movie_genre UNIQUE (movie_id, slug);
...
DROP CONSTRAINT IF EXISTS series_genres_series_id_tmdb_genre_id_key;
...
-- Add new unique constraint and index on slug
ALTER TABLE tvshow.series_genres
ADD CONSTRAINT uq_series_genre UNIQUE (series_id, slug);
