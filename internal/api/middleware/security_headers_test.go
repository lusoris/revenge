...
func TestSecurityHeadersMiddleware(t *testing.T) {
	...
	tests := []struct {
		...
		{"Content-Security-Policy", "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' image.tmdb.org *.image.tmdb.org data:; media-src 'self' blob:; connect-src 'self'; font-src 'self'; object-src 'none'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"},
