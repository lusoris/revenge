...
func newTestMovie() *Movie {
	return &Movie{
		...
		TMDbID:    new(int32(550)),
		IMDbID:    new("tt0137523"),
		Title:     "Fight Club",
		Year:      new(int32(1999)),
		Runtime:   new(int32(139)),
		Overview:  new("A depressed man suffering from insomnia meets a strange soap salesman."),
		CreatedAt: time.Now(),
		UpdatedAt: time.Now(),
	}
}
...
func TestService_GetMovieCrew(t *testing.T) {
	...
	movieID := uuid.Must(uuid.NewV7())
	crew := []MovieCredit{
		{ID: uuid.Must(uuid.NewV7()), MovieID: movieID, Name: "David Fincher", CreditType: "crew", Job: new("Director")},
	}
