...
# Revenge API Compatibility Reference

> Translating C# Revenge API patterns to Go

## Goal: 100% API Compatibility

Every endpoint must match the original C# Revenge:
...
```csharp
// Revenge.Api/Controllers/UserController.cs
[Route("Users")]
public class UserController : BaseRevengeApiController
{
...
```go
// Revenge date format
const RevengeDateFormat = "2006-01-02T15:04:05.0000000Z"

func formatDate(t time.Time) string {
    return t.UTC().Format(RevengeDateFormat)
}
```
...
```go
func TestGetUserByID_MatchesRevenge(t *testing.T) {
    // Response must match Revenge exactly
    want := `{
        "Id": "...",
        "Name": "TestUser",
        "HasPassword": true,
...
1. Find the C# controller in `Revenge.Api/Controllers/`
2. Note the route, attributes, parameters, response types
3. Check DTOs in `MediaBrowser.Model/` and `Revenge.Api/Models/`
