...
> Fetched: 2026-01-30T23:53:35.640697+00:00
> Content-Hash: bddc7f619f2c0c96
> Type: html
...
is set to NOPTS. This is ignored when the
...
delayed bt the time duration specified in
...
file), and the duration of the files (if not specified by the
...
is an unique identifier, possibly small and
consecutive.
...
packet metadata values which are the start_time and
...
The title number to play. Must be set if
...
The entry PGC to start playback, in conjunction with
...
Default is 0, automatically resolve from value of
...
The entry PG to start playback, in conjunction with
...
stored on a hard drive. Not compatible with
...
is set to 1, then the loop setting
...
as the GIF in this example loops
infinitely.
...
d" is used, the string representing the number in each
filename is 0-padded and

N

is the total number of 0-padded
digits representing the number. The literal character ’%’ can be
specified in the pattern with the string "%%".

If the sequence pattern contains "%d" or "%0

N

d", the first filename of
the file list specified by the pattern must contain a number
inclusively contained between
...
, etc.; the pattern "i%%m%%g-%d.jpg" will match a
...
d", for example to convert a single image file
...
file in the sequence, starting from
...
also available for other filters (see

drawtext

filter for examples). Default
...
By default, for backward compatibility, the MCC demuxer extracts just the EIA-608 and CEA-708 closed captions and returns a

EIA_608

stream, ignoring all other VANC data.
You can change it to return all VANC data in a

SMPTE_436M_ANC

data stream by setting
...
This retains all VANC data and inserts it into the output MXF file as a
...
box.
Default is enabled, which will prefer to use the

tfdt

box to set DTS. Disable to use the
...
box.
In either case, the timestamp from the

mfra

box will be used if it’s available and
...
Export unrecognized boxes within the

udta

box as metadata entries. The first four
characters of the box type are set as the key. Default is false.

export_xmp

Export entire contents of
...
is set and this option isn’t, the contents of
...
which allows up to
a 10 second dts correction for 48 kHz audio streams while accommodating 99.9% of
...
For multiplane formats,

stride

is a list of line size for each
plane.
...
, has 8 bytes of padding in each line of Y plane,
...
reference for relative timestamps will be
...
Store raw video frames with the ‘
...
CDMA2000 multimedia services, similar to ‘
...
, which has to be fulfilled for any
...
create fragments that contain up to
...
do not create fragments that are shorter than
...
integrating libavformat, not from
...
This option is implicitly set when writing ‘
...
Range is 1 to INT_MAX. Default is
...
native stream time base. Default is
...
these values can be computed.  Default is
...
NTP field in the PRFT box. Set value as ‘

wallclock

’ to specify timesource
as wallclock time and ‘

pts

’ to specify timesource as input packets’ PTS
values.
...
to force writing a timecode track,
...
’ is applicable only for a live encoding use case,
where PTS values are set as as wallclock time at the source. For example, an
encoding use case with decklink capture source where
...
<normal input/transcoding options>

-movflags isml+frag_keyframe -f ismv http://server/publishingpoint.isml/Streams(Encoder1)
...
specify how many times to play the content,
...
’ variant should be selected for streaming.
...
(ffmpeg-filters)the "amerge" section in the ffmpeg-filters manual
...
number of times to loop an animated AVIF,
...
enables silence detection. Default is
...
Threshold for detecting silence. Range is from
...
detection can only be used with version
...
is a hexadecimal number 0-padded to
...
identifiers, an ffmpeg-specific

$ext$

identifier is also
supported. When specified,
...
in the file name with muxing format’s extensions such as
...
A few examples of possible values for the
...
option, if not specified assume the basename
...
value can be set). Applicable only when the
...
options are enabled. This is an informative fields
...
option is enabled. It is set to
...
containing a video stream encoded through ‘
...
option keeps only the latest 5 segments with
...
’ video stream and at most two
‘

pcm_s16

’ audio streams. More constraints are defined by the
...
This muxer writes the streams metadata in the ‘
...
(ffmpeg-formats)the Metadata chapter
...
to extract metadata from an input file to a
...
’ pseudo-muxer allows the separation of encoding and
...
muxer and can be used to send data to several
...
’ muxer if the queue fills up or if the
...
part of the stream. By default this option is set to
...
recovery is not attempted even when the
...
the packets corresponding to the
...
output. Note that the value of the
...
It accepts at maximum one ‘

cinepak

’ or raw video stream, and at
...
write the file header if set to
...
together with several pictures used with ‘
...
, converted to raw audio and video packets, and store it
in the file
...
is a hexadecimal number representing the computed hash
for the packet.
...
ends with a delay until the next frame. The default is
...
It accepts at most one video stream with codec ‘
...
’, and several audio streams with rate 48000Hz
and codec ‘
...
is a short string representing
...
video, and store it in the file
...
muxer, which provides a more generic and
...
segment and subtitle filenames. In any case, if
...
Set the start numbers according to the
...
to every entry in the playlist.
Useful to generate playlists with absolute paths.

Note that the playlist sequence number must be unique for each segment
and it is not to be confused with the segment filename sequence number
which can be cyclic, for example if the
...
Set the segment filename. Unless the
...
is used as a string format with the
...
may contain a full path or relative path specification,
...
is set, the whole expanded value of
...
pattern must contain the string "%v", and this string will be
...
to expand the segment filename with
...
file-20160215-1455569023.ts

,

file-20160215-1455569024.ts

, etc.
Note: On some systems/environments, the
...
file-20160215-0001.ts

,

file-20160215-0002.ts
...
, it will create all subdirectories which
are present in the expanded values of option
...
20160215/file-20160215-1455569024.ts
...
will create a directory hierarchy

2016/02/15

(if any of them do not
exist), and then produce the playlist,
...
2016/02/15/file-20160215-1455569024.ts
...
for segment encryption. The first
line of

key_info_file

specifies the key URI written to the playlist. The
key URL is used to access the encryption key during playback. The second line
specifies the path to the key file used to obtain the key during the encryption
process. The key file is read as a single packed array of 16 octets in binary
format. The optional third line specifies the initialization vector (IV) as a
hexadecimal string to be used instead of the segment sequence number (default)
for encryption. Changes to

key_info_file

will result in segment
...
is expanded to the segment filename with localtime.

For example:

ffmpeg -i in.nut -hls_segment_type fmp4 -strftime 1 -hls_fmp4_init_filename "%s_init.mp4" out.m3u8

will produce init like this
...
pattern must contain the string "%v", this string specifies
...
tag to playlists that has video segments
and can play only I-frames in the
...
option expression besides date/time values when

strftime

option is on. To get fixed width numbers with trailing zeroes, %%0xd format
...
Write segment data to

filename.tmp

and rename to filename only once the
segment is complete.
...
Master playlist files specified with
...
to 0; the playlist must not change.
...
method, and update the m3u8 files every
...
with bitrate 256k. Here, three media playlist with file names
...
http://example.com/live/vs_0/out.m3u8
...
is not set, then the first available ccgroup in

cc_stream_map

is mapped to the output variant stream.
...
creates an HLS master playlist with name
...
’ muxer writes video frames to image files.

The output filenames are specified by a pattern, which can be used to
produce sequentially numbered series of files.
The pattern may contain the string "%d" or "%0

N

d", this string
specifies the position of the characters representing a numbering in
the filenames. If the form "%0

N

d" is used, the string
...
’ muxer accepts the same options as the ‘
...
for creating a sequence of files
...
d", for example to create a single image file

img.jpeg

from the start of the input video you can employ the command:
...
option allows you to expand the filename with
...
’ is not explicitly set, it is automatically
...
value in the output. Defaults to trying to deduce from the stream’s
...
alias for the byte sequence

E1h 00h 00h 00h

.
Disabled by default because some

.mcc

files disagree on whether it has 2 or 3 zero bytes.
...
stream from a MXF file and write it to a MCC file at 30 fps.

ffmpeg -i input.mxf -c copy -map 0:d -override_time_code_rate 30 out.mcc

Extract EIA-608/CTA-708 closed captions from a

.mp4

file and write them to a MCC file at 29.97 fps.
...
single APIC frame.  The stream metadata tags
...
’. This identifies a transponder in DVB.
Default is
...
’, also known as program in DVB. Default is
...
Set the first PID for PMTs. Default is
...
Set PAT, PMT, SDT and NIT version (default

0

, valid values are from 0 to 31, inclusively).
This option allows updating stream structure so that standard consumer may
detect the change. To do so, reopen output
...
pages that are approximately

duration

microseconds long. This allows the
...
. The list type is specified by the

segment_list_type

option. The entry filenames in the segment
...
muxer, which provides a more specific
...
reference stream. The default value is
...
Generate also a listfile named
...
to each entry. Useful to generate absolute paths.
...
is the name of the output file generated by the
...
Set segment duration to

time

, the value must be a duration
...
Set minimum segment duration to
...
set to "900" this makes it possible
...
set to "300" this makes it possible to
...
. The key frame times specified by

force_key_frames

may not be set accurately because of rounding
...
Specify a list of split video frame numbers.
...
, etc., and write the list of
generated segments to
...
frame numbers sequence specified with the
...
is the index of the mapped stream,
...
output streams. So all streams intended for output must be specified using
...
(ffmpeg-utils)the "Quoting and escaping"
...
pairs separated by ’:’, between square brackets. If
...
must be a stream specifier (see
...
to three different destinations. The
...
in order to make it contain only
audio packets.

ffmpeg -i ... -map 0 -flags +global_header -c:v libx264 -c:a aac
       -f tee "[bsfs/v=dump_extra=freq=keyframe]out.ts|[movflags=+faststart]out.mp4|[select=a]out.aac"

As above, but select only stream
...
’ string, followed by a version number (now 1).
...
Note that whitespace in metadata (e.g. ‘

foo = bar

’) is considered to be
a part of the tag (in the example above key is ‘
