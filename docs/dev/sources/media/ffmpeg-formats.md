...
Export entire contents of XMP_box and uuid box as a string with key `xmp`. Note that if `export_all` is set and this option isn’t, the contents of XMP_ box are still exported but with key `XMP_`. Default is false.
...
Next a chapter section must contain chapter start and end times in form ‘START=num’, ‘END=num’, where num is a positive integer.
  ...
  3. Note that whitespace in metadata (e.g. ‘foo = bar’) is considered to be a part of the tag (in the example above key is ‘foo ’, value is ‘ bar’).
