...
> Fetched: 2026-02-01T11:48:11.097209+00:00
> Content-Hash: c55942164372128d
> Type: html
...
One demuxer instance is created for each -i option, and sends encoded _packets_ to _decoders_ or _muxers_. 

In other literature, demuxers are sometimes called _splitters_ , because their main function is splitting a file into elementary streams (though some files only contain one elementary stream). 
...
A simple filtergraph is associated with an _output elementary stream_ ; it receives the input to be filtered from a _decoder_ and sends filtered output to that output stream’s _encoder_. 
...
_Transcoding_ is the process of decoding a stream and then encoding it again. Since encoding tends to be computationally expensive and in most cases degrades the stream quality (i.e. it is _lossy_), you should only transcode when you need to and perform streamcopy otherwise. Typical reasons to transcode are: 
...
When transcoding, audio and video streams can be filtered before encoding, with either a _simple_ or _complex_ filtergraph. 
...
-filter_buffered_frames nb_frames (_global_)
...
-force_key_frames[:stream_specifier] scd_metadata (_output,per-stream_)
...
Automatically crop the video after decoding according to file metadata. Default is _all_. 
...
When device and kernel_driver are not specified, use this option to specify the vendor id associated with the desired device. This option is available only when the hardware acceleration method _drm_ and _vaapi_ are enabled and _kernel_driver_ is not specified. 
...
Choose the GPU device on the second platform supporting the _cl_khr_fp16_ extension. 
...
_-init_hw_device vulkan:0,instance_extensions=VK_KHR_wayland_surface+VK_KHR_xcb_surface_
...
Directives tagged with _packet_ may only be used with -stats_enc_post_fmt and -stats_mux_pre_fmt. 
