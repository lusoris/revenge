...
> Fetched: 2026-01-30T23:53:30.973128+00:00
> Content-Hash: 1d6b32c7236d4a10
> Type: html
...
(short for "demultiplexers") read an input source in order to
extract
...
In other literature, demuxers are sometimes called
...
receive encoded (compressed)

packets

for an audio, video,
or subtitle elementary stream, and decode them into raw

frames

(arrays of
pixels for video, PCM for audio). A decoder is typically associated with (and
receives its input from) an elementary stream in a

demuxer

, but sometimes
may also exist on its own (see
...
process and transform raw audio or video

frames

. A
filtergraph consists of one or more individual

filters

linked into a
graph. Filtergraphs come in two flavors -
...
A simple filtergraph is associated with an

output elementary stream

; it
receives the input to be filtered from a

decoder

and sends filtered
output to that output stream’s
...
deinterlacer) followed by resizing (using the
...
receive raw audio, video, or subtitle
...
. The encoding (compression) process is
typically

lossy

- it degrades stream quality to make the output smaller;
some encoders are

lossless

, but at the cost of much higher output size. A
...
supported yet). Every encoder is associated with some muxer’s
...
(short for "multiplexers") receive encoded

packets

for
their elementary streams from encoders (the
...
path), interleave them (when there is more
...
with 3 elementary streams, from which we take the
second and write it to file
...
option is used twice here, creating two streams in the
output file - one fed by the first input and one by the second. The single
instance of the

-c

option selects streamcopy for both of those streams.
You could also use multiple instances of this option together with

Stream specifiers

to apply different values to each stream, as will be
...
Note how a separate instance of the

-c

option is needed for every
...
is the process of decoding a stream and then encoding it
again. Since encoding tends to be computationally expensive and in most cases
degrades the stream quality (i.e. it is

lossy

), you should only transcode
...
Note how it uses stream specifiers
...
to select input
streams and apply different values of the

-c

option to them; see the
...
option. Note that this option is global, since a
complex filtergraph, by its nature, cannot be unambiguously associated with a
single stream or file. Each instance of
...
of the other. Its audio counterpart is the
...
it to be fed back to complex filtergraphs. This is done with the
...
below. Stream handling is set via the
...
file. In particular, codec options are applied by ffmpeg after the
stream selection process and thus do not influence the latter. If no
...
If it cannot, ffmpeg will abort and
...
, since it has the greatest
number of channels.

For subtitles, it will select
...
is a Matroska container file which accepts subtitle streams, only a
video and audio stream shall be selected. The subtitle stream of
...
, a subtitle encoder is specified in the command and so, the subtitle stream is
selected, in addition to the video stream. The presence of
...
, only accepts text-based subtitle streams. So, even though
the first subtitle stream available belongs to

C.mkv

, it is image-based and hence skipped.
The selected stream,
...
is sent to the hue filter, whose output is cloned once using
...
. The overlay output isn’t labelled, so it is
sent to the first output file
...
. Since this filter
output is also unlabelled, it too is mapped to the first output file. The presence of
...
would set the
thread count for the second stream to 4. If
...
from the matching streams. Stream numbering is based on the
...
is one of following: ’v’ or ’V’ for video, ’a’ for audio, ’s’
...
. Otherwise, it matches all streams of the
specified type.
...
is used, then it matches streams which both
are part of the group and match the
...
is used, then it matches streams which both
are part of the program and match the
...
is not given, matches streams that contain the given tag with any
value. The colon character ’:’ in
...
is a list of
one or more dispositions (as printed by the
...
syntax cannot be used for boolean
AVOptions, use
...
output after its duration reaches
...
is enabled (the default), this
extra segment between the seek point and

position

will be decoded and
discarded. When doing stream copy or when
...
the time duration specified in
...
on streams, chapters or programs. See
...
is a sequence of disposition flags separated by ’+’ or ’-’. A ’+’
...
. If the first flag is not prefixed, the resulting
disposition is

value

. It is also possible to clear the disposition by
setting it to 0.
...
ping an input group, adding the specified
...
s that belong to all IAMF Audio Element the same
...
being automatically selected or mapped for any output. See
...
is a description of the filtergraph to apply to
...
Upon reinitialization, existing filter state is lost, like e.g. the frame count
...
It is on by default, to explicitly disable it you need to specify
...
the encoding process. It is made of "
...
generate timestamps assuming constant frame rate
...
option used for some input formats
...
is the stream frame rate. No data is
...
It cannot be set together with
...
is a decimal number specifying the amount in degree by
...
to disable it. When autoscale is
...
(ffmpeg-scaler)ffmpeg-scaler manual
...
.
The metadata can be added by filters like
...
.
Avoid inserting filters that duplicate frames after
...
is the number of the Direct3D 11 display adapter.
...
is not specified, use this option to specify the name of the kernel
driver associated with the desired device. This option is available only when
the hardware acceleration method
...
are not specified, use this option to specify
...
platform-appropriate subdevice (‘
...
’ is used
as default subdevice type when
...
is specified at
configuration time. On Linux user can use ‘
...
-init_hw_device vaapi=va:/dev/dri/renderD129 -init_hw_device qsv=hw1@
...
system-dependent list of devices.  If
...
This can be used to set the device to upload to with the

hwupload

filter,
or the device to map to with the
...
0 to disable all guessing. Using the
...
input file (specified with

-i

), the second takes an output from some
complex filtergraph (specified with
...
may be given after the stream specifier, which
...
may be set to ’all’ to use all the views
...
select a view by its display position;
...
parameters specify, which metadata to copy.
...
is a stream specifier as described
in the

Stream specifiers

chapter. In an input metadata specifier, the first
...
between frames â this is typically the case for subtitles).

This option controls the maximum duration of buffered frames in seconds.
Larger values may allow the
...
is automatically disabled when employing the
...
, ffmpeg will remove the discontinuity by
...
, ffmpeg will drop the PTS/DTS timestamp
value.
...
MP4-encapsulated H.264 stream to Annex B) to the
...
subtitle stream. Note, however, that since both
examples use
...
To connect an input stream, use
...
may be followed by the view specifier, see documentation for the

-map

option for its syntax.

To connect a loopback decoder use [dec:
...
(line 3) encodes one scaled output with
...
option. It is enabled by default, so seeking is accurate when
transcoding. Use
...
option is considered an actual timestamp, and is not
...
Any input stream can be fully discarded, using value
...
graphs, including those defined by
...
writes information about packets just as they are about to
be sent to the muxer. Every frame or packet produces one line in the specified
file. The format of this line is controlled by
...
), as an integer. Should
be multiplied by

tbi

to compute presentation time. Printed as
...
used for any kind of codec. For the
...
folder along the executable on win32,
...
suffix .avpreset in the directories
...
to enable LAME support by passing
...
above command in combination with the
...
will contain the first four streams
