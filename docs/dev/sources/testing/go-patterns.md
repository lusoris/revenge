...
> Fetched: 2026-01-30T23:54:55.415934+00:00
> Content-Hash: b7c04b0188c1105d
> Type: html
...
This topic is part of a multi-part tutorial that begins
  with
...
Go's built-in support for unit testing makes it easier to test as you go.
  Specifically, using naming conventions, Go's
...
function. Test function names have the form
...
says something about the specific test. Also, test
        functions take a pointer to the
...
as a parameter. You use this parameter's methods for reporting
...
value with which the function should be
            ...
            the name you passed in), you use the
...
command executes test functions (whose names
      begin with

Test

) in test files (whose names end with
      _test.go). You can add the

-v

flag to get verbose output that
...
test function checks the return value for
      the name you specified as a

Hello

function parameter. To view
      a failing test result, change the
...
function. Note that the highlighted lines change the
      value that the function returns, as if the
...
At the command line in the greetings directory, run
...
flag. The
      ...
      useful when you have a lot of tests. The
