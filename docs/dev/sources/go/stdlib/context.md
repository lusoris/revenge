...
> Fetched: 2026-01-30T23:31:53.232536+00:00
> Content-Hash: e45ae86776c930fd
> Type: html
...
it with a derived Context created using
...
. Calling the CancelFunc directly cancels the child and its
...
, which takes an error and records it as
the cancellation cause. Calling
...
. The Context should be the first
...
, even if a function permits it. Pass
...
for example code for a server that uses
Contexts.
...
call cancel as soon as the operations running in this
...
WithDeadlineCause behaves like

WithDeadline

but also sets the cause of the
returned Context when the deadline is exceeded. The returned
...
WithTimeoutCause behaves like

WithTimeout

but also sets the cause of the
returned Context when the timeout expires. The returned
...
type CancelCauseFunc func(cause

error

)

A CancelCauseFunc behaves like a

CancelFunc

but additionally sets the cancellation cause.
This cause can be retrieved by calling

Cause

on the canceled Context or on
...
Background returns a non-nil, empty

Context

. It is never canceled, has no
...
TODO returns a non-nil, empty

Context

. Code should use context.TODO when
...
on the returned context returns nil.