...
> Fetched: 2026-01-30T23:32:02.480795+00:00
> Content-Hash: 3ff8ddcf65fe5182
> Type: html
...
exclusion locks. Other than the
...
func OnceValues[T1, T2

any

](f func() (T1, T2)) func() (T1, T2)
...
Each Cond has an associated Locker L (often a
...
“synchronizes before” any Wait call
that it unblocks.
...
contention compared to a Go map paired with a separate
...
, Map arranges that a write operation
...
is a write operation when it returns swapped set to true;
and

Map.CompareAndDelete

is a write operation when it returns deleted set to true.
...
“synchronizes before” the m'th call to
...
is equivalent to a call to Lock.
...
is not associated with a particular goroutine.
...
first time for this instance of
...
, a call to Put(x) “synchronizes before”
a call to
...
If any goroutine calls

RWMutex.Lock

while the lock is already held by
one or more readers, concurrent calls to
...
“synchronizes before” the m'th call to Lock
for any n < m, just as for
...
As with Mutexes, a locked

RWMutex

is not associated with a particular
goroutine. One goroutine may
...
arrange for another goroutine to
...
goroutine, by calling

WaitGroup.Go

and then wait for all tasks to
complete by calling
...
This pattern is common in code that predates
...
Add adds delta, which may be negative, to the

WaitGroup

task counter.
If the counter becomes zero, all goroutines blocked on
...
Go calls f in a new goroutine and adds that task to the
...
Wait blocks until the

WaitGroup

task counter is zero.