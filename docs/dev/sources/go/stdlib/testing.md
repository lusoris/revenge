...
> Fetched: 2026-01-30T23:32:00.259400+00:00
> Content-Hash: b5237e9370e09b5b
> Type: html
...
New benchmarks should prefer using
...
or by storing files in the directory testdata/fuzz/<Name>
...
within the fuzz test is considered the fuzz
target. A fuzz target must accept a

*T

parameter, followed by one or more
parameters for random inputs. The types of arguments passed to

F.Add

must
be identical to the types of these parameters. The fuzz target may signal
that it's found a problem the same way tests do: by calling

T.Fail

(or any
method that calls it like
...
and seed inputs from testdata/fuzz/<Name>. In this
...
for documentation about fuzzing.
...
method can be used in a fuzz target if the input is invalid,
...
methods allow defining subtests and sub-benchmarks,
...
code that may be passed to

os.Exit

. If TestMain returns, the test wrapper
will pass the result of m.Run to
...
explicitly. Command line flags are always parsed by the time test
...
to 1 during its measurement and will restore
it before returning.
...
func Main(matchString func(pat, str
...
Systems simulating "go test" should be updated to use
...
func RunBenchmarks(matchString func(pat, str
...
func RunExamples(matchString func(pat, str
...
func RunTests(matchString func(pat, str
...
B is a type passed to

Benchmark

functions to manage benchmark
...
The other reporting methods, such as the variations of
...
and uses Cleanup to restore the current
...
(which then runs all deferred calls in the
current goroutine).
...
functions,
because they have global effect. It should also not call
...
SetParallelism sets the number of goroutines used by
...
If f depends on testing flags, then

Init

must be used to register
those flags before calling Benchmark and before calling
...
and entries in the testdata/fuzz/<FuzzTestName> directory. After
any necessary setup and calls to

F.Add

, the fuzz test must then call

F.Fuzz

to provide the fuzz target. See the testing package documentation
for an example, and see the
...
*F methods can only be called before

F.Fuzz

. Once the test is
executing the fuzz target, only

*T

methods can be used. The only *F methods
...
ff must be a function with no return value whose first argument is
...
method instead. The only

*F

methods that are allowed in
the F.Fuzz function are
...
should be called exactly once, unless
...
func MainStart(deps testDeps, tests []
...
method, must be called only from the goroutine running the
Test function.
