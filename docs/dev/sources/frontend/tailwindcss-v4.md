...
> Fetched: 2026-01-30T23:53:19.235650+00:00
> Content-Hash: 6bc338a774f5f827
> Type: html
...
— where full builds are up to 5x faster, and incremental builds are over 100x faster — and measured in microseconds.

Designed for the modern web

— built on cutting-edge CSS features like cascade layers, registered custom properties with
...
— fewer dependencies, zero configuration, and just a single line of code in your CSS file.
...
— all of your template files are discovered automatically, with no configuration required.
...
— all of your design tokens exposed as native CSS variables so you can access them anywhere.

Dynamic utility values and variants

— stop guessing what values exist in your spacing scale, or extending your configuration for things like basic data attributes.

Modernized P3 color palette

— a redesigned, more vivid color palette that takes full advantage of modern display technology.

Container queries

— first-class APIs for styling elements based on their container size, no plugins required.
...
— a new variant you can use to create enter and exit transitions, without the need for JavaScript.

not-* variant

— style an element only when it doesn't match another variant, custom selector, or media or feature query.
...
Start using Tailwind CSS v4.0 today by

installing it in a new project

, or playing with it directly in the browser on

Tailwind Play

.

For existing projects, we've published a comprehensive
...
Here are the median build times we saw when we benchmarked Tailwind CSS v4.0 against
...
Incremental rebuild with new CSS
...
Incremental rebuild with no new CSS
...
The most impressive improvement is on incremental builds that don't actually need to compile any new CSS — these builds are over 100x faster and complete in

microseconds

. And the longer you work on a project, the more of these builds you run into because you're just using classes you've already used before, like
...
— giving us more control than ever over how different style rules interact with each other.

Registered custom properties

— making it possible to do things like animate gradients, and significantly improving performance on large pages.

color-mix()

— which lets us adjust the opacity of any color value, including CSS variables and
...
— simplifying RTL support and reducing the size of your generated CSS.
...
— you can start using the framework without configuring anything, not even the paths to your template files.
...
rules for you out of the box, and use

Lightning CSS

under the hood for vendor prefixing and modern syntax transforms.
...
You know how you always had to configure that annoying
...
For example, we automatically ignore anything in your
...
And if you ever need to explicitly add a source that's excluded by default, you can always add it with the
...
"../node_modules/@my-company/ui-lib"
...
Learn more about in our new documentation on
...
you'd have to configure another plugin like
...
The new CSS-first configuration lets you do just about everything you could do in your

tailwind.config.js

file, including configuring your design tokens, defining custom utilities and variants, and more.

To learn more about how it all works, read the new
...
This makes it easy to reuse these values as inline styles or pass them to libraries like
...
We've simplified the way many utilities and variants work in v4.0 by effectively allowing them to accept certain types of arbitrary values,

without

the need for any configuration or dropping down to the arbitrary value syntax.
...
We've upgraded the entire default color palette from
...
We've brought container query support into core for v4.0, so you don't need the
...
Like our regular breakpoint variants, you can also stack
...
We've finally added APIs for doing 3D transforms, like
...
Linear gradients now support angles as values, so you can use utilities like
...
We've added the ability to control the color interpolation mode for gradients using a modifier, so a class like

bg-linear-to-r/srgb

interpolates using sRGB, and
...
Using polar color spaces like OKLCH or HSL can lead to much more vivid gradients when the
...
These new utilities work alongside the existing
...
variant adds support for the new CSS
...
, you can finally animate elements as they appear on the page without the need for any JavaScript at all.

Browser support

probably isn't quite there yet for most teams, but we're getting close!
...
variant which finally adds support for the CSS
...
— making it possible to stack up to four layers of box shadows on a single element.
...
— for auto-resizing textareas without writing a single line of JavaScript.
...
— so you can finally get rid of those ugly light scrollbars in dark mode.
...
— for carefully tweaking variable fonts that support different widths.
...
— for styling non-interactive elements marked with the
...
variant to also target open popovers.

New

descendant variant

— for styling all descendant elements, for better or for worse.

Check out the relevant documentation for all of these features to learn more.

And that's it — that's Tailwind CSS v4.0.

It's been years of work to get to this point, but we're all extremely proud of this release and we can't wait to see what you build with it.
