...
This page covers core concepts in OPA's policy language ([Rego](/docs/policy-language)) as well as how to download and run OPA.

## What is OPA?​

OPA [decouples](/docs/philosophy#policy-decoupling) policy decision-making from policy enforcement. When your software needs to make policy decisions it **queries** OPA and supplies structured data (e.g., JSON) as input. OPA accepts arbitrary structured data as input.
...
OPA policies are expressed in a high-level declarative language called Rego. Rego (pronounced "ray-go") is purpose-built for expressing policies over complex hierarchical data structures. For detailed information on Rego see the [Policy Language](/docs/policy-language) documentation.
...
They can also be run locally on your machine using the `opa eval` command, here are setup instructions.

note

This section covers the building blocks of writing policies in Rego. You can see how these concepts come together to solve our network security policy in the Complete Example.
...
The simplest way to interact with OPA is via the command-line using the [`opa eval` sub-command](/docs/cli#eval). It is a swiss-army knife that you can use to evaluate arbitrary Rego expressions and policies. `opa eval` supports a large number of options for controlling evaluation. Commonly used flags include:
