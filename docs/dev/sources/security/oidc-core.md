...
Calculate `sub` = SHA-256 ( sector_identifier || local_account_id || salt ).
  ...
  1. The Sector Identifier can be concatenated with a local account ID and a salt value that is kept secret by the Provider. The concatenated string is then encrypted using an appropriate algorithm.
  
Calculate `sub` = AES-128 ( sector_identifier || local_account_id || salt ).
  
  1. The Issuer creates a Globally Unique Identifier (GUID) for the pair of Sector Identifier and local account ID and stores this value.
