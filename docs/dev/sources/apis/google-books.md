...
---
  ...
  * [ Guides ](https://developers.google.com/books/docs/overview)



#  Using the API Stay organized with collections  Save and categorize content based on your preferences. 
...
## Contents
  ...
  1. Introduction
     ...
     2. Google Books IDs
     3. Setting User Location
  2. Working with volumes
     1. Performing a search
     2. Retrieving a specific volume
  3. Working with bookshelves
     1. Retrieving a list of a user's public bookshelves
     2. Retrieving a specific public bookshelf
     3. Retrieving a list of volumes on a public bookshelf
  4. Working with bookshelves in "My Library"
     1. Retrieving a list of my bookshelves
     2. Retrieving a list of volumes on my bookshelf
     3. Adding a volume to my bookshelf
     4. Removing a volume from my bookshelf
     5. Clearing all volumes from my bookshelf
  5. Query parameter reference
     1. Standard query parameters
     2. API-specific query parameters
...
If you're unfamiliar with Google Books concepts, you should read [Getting Started](/books/docs/v1/getting_started) before starting to code.
...
**Tip:** The Google APIs client libraries can handle some of the authorization process for you. They are available for a variety of programming languages; check the [page with libraries and samples](/books/docs/v1/libraries) for more details.
...
To keep your API keys secure, follow the [best practices for securely using API keys](//cloud.google.com/docs/authentication/api-keys).
...
You need to specify ID fields with certain API method calls. There are three types of IDs used within Google Books: 
  ...
  * **Bookshelf IDs** \- Numeric values given to a bookshelf in a user's library. Google provides some pre-defined shelves for every user with the following IDs: 
    ...
    * Recently Viewed: 6
    * My eBooks: 7
    * Books For You: 8 _If we have no recommendations for the user, this shelf does not exist._
...
You can use the `projection` parameter with one of the following values to specify a predefined set of Volume fields to return:
  ...
  * `lite` \- Returns only certain fields. See field descriptions marked with double asterisks in the [Volume reference](/books/docs/v1/reference#resource_volumes) to find out which fields are included.
