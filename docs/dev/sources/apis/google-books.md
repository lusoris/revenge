...
> Fetched: 2026-01-30T23:50:42.837349+00:00
> Content-Hash: 5a4ba743197762a0
> Type: html
...
The Google Books API allows developers to search and access book content, as well as create and view personalization around that content.

Requests to the API require identification using an OAuth 2.0 token for authorized requests or an API key for public data.

The API uses specific IDs for volumes, bookshelves, and users, which are consistent with the IDs used on the Google Books website.

The API supports various operations for working with volumes, such as performing searches and retrieving specific volumes, with optional parameters for filtering, pagination, print type, projection, and sorting.

The API also provides methods for working with bookshelves, including retrieving lists of public and private bookshelves, retrieving specific bookshelves, and managing volumes on "My Library" bookshelves.
...
Every request your application sends to the Books API needs to identify your application to Google. There are two ways to identify your application: using an

OAuth 2.0 token

(which also authorizes the request) and/or using the application's
...
to authorize requests. No other authorization protocols are supported. If your application uses
...
- Unique strings given to each volume that Google Books
  knows about. An example of a volume ID is

_LettPDhwR0C

. You can
  use the API to get the volume ID by making a request that returns a Volume
  resource; you can find the volume ID in its
...
- Numeric values given to a bookshelf in a user's
  library. Google provides some pre-defined shelves for every user with the
  following IDs:
...
If we have no recommendations for the user,
      this shelf does not exist.

Custom shelves have IDs greater than 1000. A bookshelf ID is unique for a
  given user, i.e., two users can have a bookshelf with the same ID that refer
  to different bookshelves. You can use the API to get the bookshelf ID by
  making a request that returns a Bookshelf resource; you can find the bookshelf
  ID in its
...
- Unique numeric values assigned to each user. These
  values are not necessarily the same ID value used in other Google services.
  Currently, the only way retrieve the user ID is to extract it from the
  selfLink in a Bookshelf resource retrieved with an authenticated request.
  Users can also obtain their own user ID from the Books site.  A user cannot
  obtain the user ID for another user via the API or the Books site; the other
  user would have to share that information explicitly, by email for example.
...
URL parameter. Here is an example:

https://books.google.com/ebooks?

id=buc0AAAAMAAJ

&dq=holmes&as_brr=4&source=webstore_bookcard
...
&num=10&uid=11122233344455566778&source=gbs_slider_cls_metadata_0_mylibrary
...
- Search for volumes that contain this text string. There are
special keywords you can specify in the search terms to search in particular
fields, such as:

intitle:

Returns results where the text following this
...
request. However, if the call is made with authentication, each
...
HTTP status code and the volume results:
...
parameter to   restrict the returned results to volumes that have an available
download format of
...
GET https://www.googleapis.com/books/v1/volumes?q=pride+prejudice&download=epub&
...
parameter to  restrict the returned results further by setting it the to one of the following
values:

partial

- Returns results where at least parts of the text
  are previewable.

full

- Only returns results where all of the text is
  viewable.

free-ebooks

- Only returns results that are free Google
  eBooks.

paid-ebooks

- Only returns results that are Google eBooks
  with a price.

ebooks

- Only returns results that are Google eBooks, paid or
  free. Examples of non-eBooks would be publisher content that is available in
...
- The position in the collection at which to
  start. The index of the first item is 0.

maxResults

- The maximum number of results to return. The
  default is 10, and the maximum allowable value is 40.
...
parameter to restrict the returned results to a specific print or publication
...
GET https://www.googleapis.com/books/v1/volumes?q=time&printType=magazines&
...
- Returns only certain fields. See field descriptions
  marked with double asterisks in the
...
is the parameter used in pagination (above),
...
- Returns results in order of the relevance of
  search terms (this is the default).

newest

- Returns results in order of most recently to least
  recently published.
...
path parameter with the ID of the volume to retrieve. See the
...
request that gets a single volume:
...
authentication, so you do not have to provide the
...
request. However, if the call is made with
...
section is of particular interest in determining
what features are available for an eBook. An
...
property indicating if this type of ebook is available.
...
domain in the user's location. A

pdf

for Google books indicates a
scanned pages version of the ebook with similar details such as if it is
available and a download link. Google recommends
...
section, the volume is not available as a
Google eBook.
...
GET https://www.googleapis.com/books/v1/volumes/zyTCAlFPjgYC?projection=lite&
...
section for more information on user IDs.

Request

Here is an example:

GET https://www.googleapis.com/books/v1/users/
...
public bookshelves, you do not have to provide the
...
HTTP status code and the list of bookshelves:
...
path parameters with the IDs that specify
...
HTTP status code and the bookshelf resource:
...
request the a URI with the following format:

https://www.googleapis.com/books/v1/user/
...
, you can use the following query parameter when retrieving a list of volumes on a public bookshelf.
...
bookshelves. So you must provide the
...
path parameter with the ID of
the bookshelf. See the
...
"My Library" volumes. So you must provide the
...
, you can use the following query parameter when retrieving a list of volumes on one of the authenticated user's bookshelves.
...
POST https://www.googleapis.com/books/v1/mylibrary/bookshelves/

0

/addVolume?volumeId=NRWlitmahXkC&
...
bookshelf, so you must provide the
...
request. No data, however, is required with this
...
when adding a volume to one of the authenticated user's bookshelves.
...
/removeVolume?volumeId=NRWlitmahXkC&
...
when removing a volume from one of the authenticated user's bookshelves.
...
when clearing all volumes from one of the authenticated user's bookshelves.
...
- Restrict results to volumes where at least part of the text are previewable.
...
- Restrict results to Google eBooks, paid or free.Examples of non-eBooks would be publisher content that is available in
          limited preview and not for sale, or magazines.
...
Restrict the search results to those with a certain language by specifying

langRestrict

to a two-letter ISO-639-1 code, such as "en" or "fr".
...
For any request for all items in a collection, you can paginate results by specifying
...
in the parameters for the request.
...
is the parameter used in pagination, ordered by
       most relevant first.
...
- Returns search results in order of
...
- Includes only a subject of volume and access metadata.
...
When creating a query, list search terms separated by a '+', in the form
...
. (Alternatively, you can separate
      them with a space, but as with all of the query parameter values, the spaces
      must then be URL encoded.) The API returns all entries that match all of the
      search terms (like using AND between terms). Like Google's web search, the
      API searches on complete words (and related words with the same stem), not
      substrings.
...
but don't contain the word

"Austen"

, use the following query parameter value:
...
For any request for all items in a collection, you can paginate results
      by specifying
