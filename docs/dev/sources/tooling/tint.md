...
> Fetched: 2026-01-30T23:49:26.243277+00:00
> Content-Hash: ecdc1e641d1671ad
> Type: html
...
Package tint implements a zero-dependency

slog.Handler

that writes tinted
(colorized) logs. The output format is inspired by the
...
The output format can be customized using
...
Attr returns a tinted (colorized)

slog.Attr

that will be written in the
specified color by the

tint.Handler

. When used with any other
...
Err returns a tinted (colorized)

slog.Attr

that will be written in red color
by the
...
NewHandler creates a

slog.Handler

that writes tinted logs to Writer w,
...
Options can be used as a drop-in replacement for
