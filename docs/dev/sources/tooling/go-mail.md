...
> Fetched: 2026-01-30T23:49:49.715659+00:00
> Content-Hash: b74da085bac12f1f
> Type: html
...
// This constant follows the recommendation of

RFC 2047

, which suggests a maximum length of 76 characters.
...
("server does not support SMTP AUTH type: SCRAM-SHA-256")
...
("server does not support SMTP AUTH type: SCRAM-SHA-256-PLUS")
...
("SMTP Auth autodiscover was not able to detect a supported " +
...
("timeout cannot be zero or negative")
...
("invalid HELO/EHLO value - must not be empty")
...
("message is 8bit unencoded, but server does not support 8BITMIME")
...
("DSN rcpt notify option can only be: NEVER, " +
...
ErrInvalidDSNRcptNotifyCombination =
...
("DSN rcpt notify option NEVER cannot be " +
...
) DialToSMTPClientWithContext(ctxDial
...
) SetDebugLogWithSMTPClient(client *
...
) SetLoggerWithSMTPClient(client *
...
// EncodingB64 represents the Base64 encoding as specified in
...
// EncodingQP represents the "quoted-printable" encoding as specified in
...
// NoEncoding represents 8-bit encoding for email messages as specified in
...
func WithFileContentType(contentType
...
func WithFileDescription(description
...
func WithFileEncoding(encoding
...
// HeaderDispositionNotificationTo is the MDN header as described in
...
// HeaderMIMEVersion represents the "MIME-Version" field as per
...
func EMLToMsgFromFile(filePath
...
func EMLToMsgFromReader(reader
...
func EMLToMsgFromString(emlString
...
The content parameter should be an

RFC 822

-style email body. The lines of content should be CRLF terminated.
...
) AddAlternativeHTMLTemplate(tpl *
...
) AddAlternativeString(contentType
...
) AddAlternativeTextTemplate(tpl *
...
recipients being aware of it. The provided address is validated according to
...
, and an error will be returned if the validation fails.
...
in the "TO" field. The provided address is validated according to

RFC 5322

, and an error will be returned if
...
client. The provided address is validated according to

RFC 5322

, and an error will be returned if the
validation fails.
...
visible in the mail client. The provided name and address are validated according to
...
to this method. Each provided address is validated according to
...
Each address is validated according to

RFC 5322

and set as a recipient in the "BCC" field. If any validation
...
with caution if address validation is critical, as invalid addresses are determined according to
...
address is validated according to

RFC 5322

, and an error will be returned if ANY validation fails.
...
and set as a recipient in the "CC" field. If any validation
...
caution if address validation is critical, as invalid addresses are determined according to
...
The provided address is validated according to

RFC 5322

and will return an error if the validation fails.
...
The provided name and address are validated according to

RFC 5322

and will return an error if
the validation fails.
...
from address (if available) for sending. The provided address is validated according to
...
MDN. The provided address is validated according to

RFC 5322

standards. If the address is invalid,
...
) RequestMDNAddToFormat(name, addr
...
standards. If the provided address is invalid, an error will be returned.
...
Notification (MDN) from the receiving end, as specified in
...
standards. If ANY address is invalid, an error
...
Address validation is performed according to

RFC 5322

standards. If the provided address is invalid,
...
Addresses are parsed according to

RFC 5322

. If parsing any of the provided values fails,
...
An error if parsing the address according to
...
) SetAddrHeaderFromMailAddress(header
...
Since we expect the mail.Address instances to be already parsed according to
...
) SetAddrHeaderIgnoreInvalid(header
...
. If parsing of any of the provided values fails,
...
SetDate sets the "Date" header for the Msg to the current time in a valid

RFC 1123

format.

This method retrieves the current time and formats it according to

RFC 1123

, ensuring that the "Date"
...
SetDateWithValue sets the "Date" header for the Msg using the provided time value in a valid

RFC 1123

format.

This method takes a `time.Time` value as input and formats it according to
...
) SetGenHeaderPreformatted(header
...
the message header, we cannot guarantee any compliance with

RFC 2822

. It is advised to use SetGenHeader
...
) SetMessageIDWithValue(messageID
...
) SignWithTLSCertificate(keyPairTLS *
...
and set as a recipient in the "TO" field. If any validation
...
address validation is critical. Invalid addresses are determined according to
...
) WriteToSendmailWithCommand(sendmailPath
...
) WriteToSendmailWithContext(ctx
...
func WithMiddleware(middleware
...
WithDSN enables DSN (Delivery Status Notifications) for the Client as described in
...
func WithDSNMailReturnType(option
...
WithDSNMailReturnType enables DSN (Delivery Status Notifications) for the Client as described in
...
func WithDSNRcptNotifyType(opts ...
...
WithDSNRcptNotifyType enables DSN (Delivery Status Notifications) for the Client as described in
...
func WithDialContextFunc(dialCtxFunc
...
func WithSMTPAuthCustom(smtpAuth
...
func WithPartContentDescription(description
...
func WithPartEncoding(encoding
...
// SMTPAuthCramMD5 is the "CRAM-MD5" SASL authentication mechanism as described in
...
// SMTPAuthPlain is the "PLAIN" authentication mechanism as described in
...
// SMTPAuthPlainNoEnc is the "PLAIN" authentication mechanism as described in
...
// SMTPAuthSCRAMSHA1 is the "SCRAM-SHA-1" SASL authentication mechanism as described in
...
// SMTPAuthSCRAMSHA1PLUS is the "SCRAM-SHA-1-PLUS" SASL authentication mechanism as described in
...
// SMTPAuthSCRAMSHA256 is the "SCRAM-SHA-256" SASL authentication mechanism as described in
...
// SMTPAuthSCRAMSHA256PLUS is the "SCRAM-SHA-256-PLUS" SASL authentication mechanism as described in
...
server supports it, as described in
...
requires that the receiving server supports this SMTP extension as described in
