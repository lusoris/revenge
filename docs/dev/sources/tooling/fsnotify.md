...
> Fetched: 2026-01-31T10:56:22.473388+00:00
> Content-Hash: 1f14c6d6980762d8
> Type: html
...
Set the FSNOTIFY_DEBUG environment variable to "1" to print debug messages to stderr. This can be useful to track down some problems, especially in cases where fsnotify is used as an indirect dependency. 

Every event will be printed as soon as there's something useful to print, with as little processing from fsnotify. 
...
[View Source](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L227)
    
    
    var (
    	// ErrNonExistentWatch is used when Remove() is called on a path that's not
    	// added.
    	ErrNonExistentWatch = [errors](/errors).[New](/errors#New)("fsnotify: can't remove non-existent watch")
    
    	// ErrClosed is used when trying to operate on a closed Watcher.
    	ErrClosed = [errors](/errors).[New](/errors#New)("fsnotify: watcher already closed")
    	...
    	//  - kqueue, fen:  Not used.
    	ErrEventOverflow = [errors](/errors).[New](/errors#New)("fsnotify: queue or buffer overflow")
    )

### Functions ¶

####  func [WithBufferSize](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L450) ¶ added in v1.7.0
    
    
    func WithBufferSize(bytes [int](/builtin#int)) addOpt

WithBufferSize sets the [ReadDirectoryChangesW](https://learn.microsoft.com/en-gb/windows/win32/api/winbase/nf-winbase-readdirectorychangesw) buffer size. 
...
####  type [Event](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L147) ¶ added in v1.0.0
...
####  func (Event) [Has](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L390) ¶ added in v1.6.0
    
    
    func (e Event) Has(op Op) [bool](/builtin#bool)

Has reports if this event has the given operation. 

####  func (Event) [String](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L393) ¶ added in v1.0.0
    
    
    func (e Event) String() [string](/builtin#string)

String returns a string representation of the event with their path. 

####  type [Op](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L173) ¶ added in v1.0.0
    
    
    type Op [uint32](/builtin#uint32)
...
####  func (Op) [Has](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L387) ¶ added in v1.6.0
    
    
    func (o Op) Has(h Op) [bool](/builtin#bool)

Has reports if this operation has the given operation. 

####  func (Op) [String](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L351) ¶ added in v1.4.0
    
    
    func (o Op) String() [string](/builtin#string)

####  type [Watcher](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L100) ¶
...
When a watched directory is removed it will always send an event for the directory itself, but may not send events for all files in that directory. Sometimes it will send events for all files, sometimes it will send no events, and often only for some files. 
...
####  func [NewBufferedWatcher](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L269) ¶ added in v1.7.0
    
    
    func NewBufferedWatcher(sz [uint](/builtin#uint)) (*Watcher, [error](/builtin#error))
...
####  func [NewWatcher](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L252) ¶
    
    
    func NewWatcher() (*Watcher, [error](/builtin#error))

NewWatcher creates a new Watcher. 

####  func (*Watcher) [Add](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L314) ¶ added in v1.0.0
    
    
    func (w *Watcher) Add(path [string](/builtin#string)) [error](/builtin#error)
...
Watching individual files (rather than directories) is generally not recommended as many programs (especially editors) update files atomically: it will write to a temporary file which is then moved to destination, overwriting the original (or some variant thereof). The watcher on the original file is now lost, as that no longer exists. 
...
####  func (*Watcher) [AddWith](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L323) ¶ added in v1.7.0
    
    
    func (w *Watcher) AddWith(path [string](/builtin#string), opts ...addOpt) [error](/builtin#error)
...
####  func (*Watcher) [Close](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L336) ¶
    
    
    func (w *Watcher) Close() [error](/builtin#error)

Close removes all watches and closes the Events channel. 

####  func (*Watcher) [Remove](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L333) ¶ added in v1.0.0
    
    
    func (w *Watcher) Remove(path [string](/builtin#string)) [error](/builtin#error)
...
####  func (*Watcher) [WatchList](https://github.com/fsnotify/fsnotify/blob/v1.9.0/fsnotify.go#L343) ¶ added in v1.5.2
    
    
    func (w *Watcher) WatchList() [][string](/builtin#string)

WatchList returns all paths explicitly added with Watcher.Add (and are not yet removed). 

The order is undefined, and may differ per call. Returns nil if Watcher.Close was called. 
