...
See [homepage](https://riverqueue.com), [docs](https://riverqueue.com/docs), and [godoc](https://pkg.go.dev/github.com/riverqueue/river), as well as the [River UI](https://github.com/riverqueue/riverui).

Being built for Postgres, River encourages the use of the same database for application data and job queue. By enqueueing jobs transactionally along with other database changes, whole classes of distributed systems problems are avoided. Jobs are guaranteed to be enqueued if their transaction commits, are removed if their transaction rolls back, and aren't visible for work _until_ commit. See [transactional enqueueing](https://riverqueue.com/docs/transactional-enqueueing) for more background on this philosophy.

#### Job args and workers ¶

Jobs are defined in struct pairs, with an implementation of [`JobArgs`](https://pkg.go.dev/github.com/riverqueue/river#JobArgs) and one of [`Worker`](https://pkg.go.dev/github.com/riverqueue/river#Worker).
...
There are some complexities around ensuring clients stop cleanly, but also in a timely manner. See [graceful shutdown](https://riverqueue.com/docs/graceful-shutdown) for more details on River's stop modes.
...
- [Batch job insertion](https://riverqueue.com/docs/batch-job-insertion) for efficiently inserting many jobs at once using Postgres `COPY FROM`.

- [Cancelling jobs](https://riverqueue.com/docs/cancelling-jobs) from inside a work function.

- [Error and panic handling](https://riverqueue.com/docs/error-handling).

- [Multiple queues](https://riverqueue.com/docs/multiple-queues) to better guarantee job throughput, worker availability, and isolation between components.

- [Periodic and cron jobs](https://riverqueue.com/docs/periodic-jobs).

- [Scheduled jobs](https://riverqueue.com/docs/scheduled-jobs) that run automatically at their scheduled time in the future.

- [Snoozing jobs](https://riverqueue.com/docs/snoozing-jobs) from inside a work function.

- [Subscriptions](https://riverqueue.com/docs/subscriptions) to queue activity and statistics, providing easy hooks for telemetry like logging and metrics.

- [Test helpers](https://riverqueue.com/docs/testing) to verify that jobs are inserted as expected.

- [Transactional job completion](https://riverqueue.com/docs/transactional-job-completion) to guarantee job completion commits with other changes in a transaction.

- [Unique jobs](https://riverqueue.com/docs/unique-jobs) by args, period, queue, and state.

- [Web UI](https://github.com/riverqueue/riverui) for inspecting and interacting with jobs and queues.

- [Work functions](https://riverqueue.com/docs/work-functions) for simplified worker implementation.
...
See [developing River](https://github.com/riverqueue/river/blob/master/docs/development.md).
...
Example_globalHooks demonstrates the use of hooks to modify River behavior which are global to a River client.
...
Example_globalMiddleware demonstrates the use of middleware to modify River behavior which are global to a River client.
...
- type JobInsertMiddlewareDefaultsdeprecated
...
- type WorkerMiddlewareDefaultsdeprecated
...
[View Source](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L42)

    const (
     FetchCooldownDefault = 100 * [time](/time).[Millisecond](/time#Millisecond)
     FetchCooldownMin     = 1 * [time](/time).[Millisecond](/time#Millisecond)
    
     FetchPollIntervalDefault = 1 * [time](/time).[Second](/time#Second)
     FetchPollIntervalMin     = 1 * [time](/time).[Millisecond](/time#Millisecond)
    
     JobTimeoutDefault  = 1 * [time](/time).[Minute](/time#Minute)
     MaxAttemptsDefault = [rivercommon](/github.com/riverqueue/river@v0.30.2/internal/rivercommon).[MaxAttemptsDefault](/github.com/riverqueue/river@v0.30.2/internal/rivercommon#MaxAttemptsDefault)
     PriorityDefault    = [rivercommon](/github.com/riverqueue/river@v0.30.2/internal/rivercommon).[PriorityDefault](/github.com/riverqueue/river@v0.30.2/internal/rivercommon#PriorityDefault)
     QueueDefault       = [rivercommon](/github.com/riverqueue/river@v0.30.2/internal/rivercommon).[QueueDefault](/github.com/riverqueue/river@v0.30.2/internal/rivercommon#QueueDefault)
     QueueNumWorkersMax = 10_000
    )

### Variables ¶

[View Source](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L11)

    var ErrJobCancelledRemotely = [rivertype](/github.com/riverqueue/river/rivertype).[ErrJobCancelledRemotely](/github.com/riverqueue/river/rivertype#ErrJobCancelledRemotely)

ErrJobCancelledRemotely is a sentinel error indicating that the job was cancelled remotely.

[View Source](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L655)
    ...
    var (
     ...
     // return this error.
     ErrNotFound = [rivertype](/github.com/riverqueue/river/rivertype).[ErrNotFound](/github.com/riverqueue/river/rivertype#ErrNotFound)
    )

### Functions ¶

#### func [AddWorker](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L102) ¶
...
#### func [AddWorkerArgs](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L113) ¶ added in v0.24.0

    func AddWorkerArgs[T JobArgs](workers *Workers, jobArgs T, worker Worker[T])

AddWorkerArgs is the same as AddWorker except that it lets args be passed explicitly rather than being instantiated implicitly. We don't know of any use for this function beyond exercising some args-related edge cases in tests are difficult/impossible to exercise otherwise, and its use should be considered internal only.

#### func [AddWorkerSafely](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L127) ¶

    func AddWorkerSafely[T JobArgs](workers *Workers, worker Worker[T]) [error](/builtin#error)
...
#### func [JobCancel](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L21) ¶

    func JobCancel(err [error](/builtin#error)) [error](/builtin#error)
...
#### func [JobSnooze](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L43) ¶

    func JobSnooze(duration [time](/time).[Duration](/time#Duration)) [error](/builtin#error)
...
A special duration of zero can be used to make the job immediately available to be reworked. This may be useful in cases like where a long-running job is being interrupted on shutdown. Instead of returning a context cancelled error that'd schedule a retry for the future and count towards maximum attempts, the work function can return JobSnooze(0) and the job will be retried immediately the next time a client starts up.

Panics if duration is < 0\.

#### func [RecordOutput](https://github.com/riverqueue/river/blob/v0.30.2/recorded_output.go#L53) ¶ added in v0.18.0

    func RecordOutput(ctx [context](/context).[Context](/context#Context), output [any](/builtin#any)) [error](/builtin#error)
...
For example, with workflows, it's common for subsequent task to depend on something done in an earlier dependency task. Consider the creation of an external resource in another API or in an database—it will typically have a unique ID that must be used to reference the resource later. A later step may require that info in order to complete its work, and the output can be a convenient way to store that info.

Output is stored in the job's metadata under the `"output"` key ([github.com/riverqueue/river/rivertype.MetadataKeyOutput](/github.com/riverqueue/river/rivertype#MetadataKeyOutput)). This function must be called within an Worker's Work function. It returns an error if called anywhere else. As with any stored value, care should be taken to ensure that the payload size is not too large. Output is limited to 32MB in size for safety, but should be kept much smaller than this.
...
Once recorded, the output is stored regardless of the outcome of the execution attempt (success, error, panic, etc.).

RecordOutput always stores output lazily as a job is being completed (whether that's completion to success or failure). Client.JobUpdate and JobUpdateTx are available to store output eagerly at any time, including from inside a work function as the job is being executed.

The output is marshalled to JSON as part of this function and it will return an error if the output is not JSON-encodable.

### Types ¶

#### type [Client](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L589) ¶

    type Client[TTx [any](/builtin#any)] struct {
...
#### func [ClientFromContext](https://github.com/riverqueue/river/blob/v0.30.2/client_context.go#L28) ¶ added in v0.0.17

    func ClientFromContext[TTx [any](/builtin#any)](ctx [context](/context).[Context](/context#Context)) *Client[TTx]
...
When testing JobArgs.Work implementations, it might be useful to use rivertest.WorkContext to initialize a context that has an available client.

The type parameter TTx is the transaction type used by the Client, pgx.Tx for the pgx driver, and *sql.Tx for the [database/sql](/database/sql) driver.

Example (Pgx) ¶

ExampleClientFromContext_pgx demonstrates how to extract the River client from the worker context when using the pgx/v5 driver. ([github.com/riverqueue/river/riverdriver/riverpgxv5](/github.com/riverqueue/river/riverdriver/riverpgxv5)).
...
#### func [ClientFromContextSafely](https://github.com/riverqueue/river/blob/v0.30.2/client_context.go#L48) ¶ added in v0.0.17

    func ClientFromContextSafely[TTx [any](/builtin#any)](ctx [context](/context).[Context](/context#Context)) (*Client[TTx], [error](/builtin#error))
...
#### func [NewClient](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L693) ¶

    func NewClient[TTx [any](/builtin#any)](driver [riverdriver](/github.com/riverqueue/river/riverdriver).[Driver](/github.com/riverqueue/river/riverdriver#Driver)[TTx], config *Config) (*Client[TTx], [error](/builtin#error))
...
#### func (*Client[TTx]) [Driver](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1330) ¶ added in v0.11.0

    func (c *Client[TTx]) Driver() [riverdriver](/github.com/riverqueue/river/riverdriver).[Driver](/github.com/riverqueue/river/riverdriver#Driver)[TTx]

Driver exposes the underlying driver used by the client.

API is not stable. DO NOT USE.

#### func (*Client[TTx]) [ID](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1612) ¶ added in v0.0.21

    func (c *Client[TTx]) ID() [string](/builtin#string)

ID returns the unique ID of this client as set in its config or auto-generated if not specified.

#### func (*Client[TTx]) [Insert](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1734) ¶

    func (c *Client[TTx]) Insert(ctx [context](/context).[Context](/context#Context), args JobArgs, opts *InsertOpts) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))
...
#### func (*Client[TTx]) [InsertMany](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1800) ¶

    func (c *Client[TTx]) InsertMany(ctx [context](/context).[Context](/context#Context), params []InsertManyParams) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))
...
Unlike with `InsertMany`, unique conflicts cannot be handled gracefully. If a unique constraint is violated, the operation will fail and no jobs will be inserted.

#### func (*Client[TTx]) [InsertManyFastTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2056) ¶ added in v0.12.0

    func (c *Client[TTx]) InsertManyFastTx(ctx [context](/context).[Context](/context#Context), tx TTx, params []InsertManyParams) ([int](/builtin#int), [error](/builtin#error))
...
Unlike with `InsertManyTx`, unique conflicts cannot be handled gracefully. If a unique constraint is violated, the operation will fail and no jobs will be inserted.

#### func (*Client[TTx]) [InsertManyTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1835) ¶

    func (c *Client[TTx]) InsertManyTx(ctx [context](/context).[Context](/context#Context), tx TTx, params []InsertManyParams) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))
...
#### func (*Client[TTx]) [InsertTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1768) ¶

    func (c *Client[TTx]) InsertTx(ctx [context](/context).[Context](/context#Context), tx TTx, args JobArgs, opts *InsertOpts) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))
...
This variant lets a caller insert jobs atomically alongside other database changes. It's also possible to insert a job outside a transaction, but this usage is recommended to ensure that all data a job needs to run is available by the time it starts. Because of snapshot visibility guarantees across transactions, the job will not be worked until the transaction has committed, and if the transaction rolls back, so too is the inserted job.

#### func (*Client[TTx]) [JobCancel](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1370) ¶ added in v0.0.17

    func (c *Client[TTx]) JobCancel(ctx [context](/context).[Context](/context#Context), jobID [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))
...
#### func (*Client[TTx]) [JobCancelTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1426) ¶ added in v0.0.17

    func (c *Client[TTx]) JobCancelTx(ctx [context](/context).[Context](/context#Context), tx TTx, jobID [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobCancelTx cancels the job with the given ID within the specified transaction. This variant lets a caller cancel a job atomically alongside other database changes. A cancelled job doesn't take effect until the transaction commits, and if the transaction rolls back, so too is the cancelled job.
...
#### func (*Client[TTx]) [JobDelete](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1443) ¶ added in v0.7.0

    func (c *Client[TTx]) JobDelete(ctx [context](/context).[Context](/context#Context), id [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobDelete deletes the job with the given ID from the database, returning the deleted row if it was deleted. Jobs in the running state are not deleted, instead returning rivertype.ErrJobRunning.

#### func (*Client[TTx]) [JobDeleteMany](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2238) ¶ added in v0.24.0

    func (c *Client[TTx]) JobDeleteMany(ctx [context](/context).[Context](/context#Context), params *JobDeleteManyParams) (*JobDeleteManyResult, [error](/builtin#error))

JobDeleteMany deletes many jobs at once based on the conditions defined by JobDeleteManyParams. Running jobs are always ignored.
...
#### func (*Client[TTx]) [JobDeleteManyTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2254) ¶ added in v0.24.0

    func (c *Client[TTx]) JobDeleteManyTx(ctx [context](/context).[Context](/context#Context), tx TTx, params *JobDeleteManyParams) (*JobDeleteManyResult, [error](/builtin#error))

JobDeleteManyTx deletes many jobs at once based on the conditions defined by JobDeleteManyParams. Running jobs are always ignored.
...
#### func (*Client[TTx]) [JobDeleteTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1456) ¶ added in v0.7.0

    func (c *Client[TTx]) JobDeleteTx(ctx [context](/context).[Context](/context#Context), tx TTx, id [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobDeleteTx deletes the job with the given ID from the database, returning the deleted row if it was deleted. Jobs in the running state are not deleted, instead returning rivertype.ErrJobRunning. This variant lets a caller retry a job atomically alongside other database changes. A deleted job isn't deleted until the transaction commits, and if the transaction rolls back, so too is the deleted job.

#### func (*Client[TTx]) [JobGet](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1465) ¶ added in v0.0.19

    func (c *Client[TTx]) JobGet(ctx [context](/context).[Context](/context#Context), id [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobGet fetches a single job by its ID. Returns the up-to-date JobRow for the specified jobID if it exists. Returns ErrNotFound if the job doesn't exist.

#### func (*Client[TTx]) [JobGetTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1475) ¶ added in v0.0.19

    func (c *Client[TTx]) JobGetTx(ctx [context](/context).[Context](/context#Context), tx TTx, id [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobGetTx fetches a single job by its ID, within a transaction. Returns the up-to-date JobRow for the specified jobID if it exists. Returns ErrNotFound if the job doesn't exist.

#### func (*Client[TTx]) [JobList](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2304) ¶ added in v0.0.17

    func (c *Client[TTx]) JobList(ctx [context](/context).[Context](/context#Context), params *JobListParams) (*JobListResult, [error](/builtin#error))
...
#### func (*Client[TTx]) [JobListTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2349) ¶ added in v0.0.17

    func (c *Client[TTx]) JobListTx(ctx [context](/context).[Context](/context#Context), tx TTx, params *JobListParams) (*JobListResult, [error](/builtin#error))
...
#### func (*Client[TTx]) [JobRetry](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1490) ¶ added in v0.0.19

    func (c *Client[TTx]) JobRetry(ctx [context](/context).[Context](/context#Context), id [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobRetry updates the job with the given ID to make it immediately available to be retried. Jobs in the running state are not touched, while jobs in any other state are made available. To prevent jobs already waiting in the queue from being set back in line, the job's scheduled_at field is set to the current time only if it's not already in the past.

MaxAttempts is also incremented by one if the job has already exhausted its max attempts.

#### func (*Client[TTx]) [JobRetryTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1507) ¶ added in v0.0.19

    func (c *Client[TTx]) JobRetryTx(ctx [context](/context).[Context](/context#Context), tx TTx, id [int64](/builtin#int64)) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobRetryTx updates the job with the given ID to make it immediately available to be retried, within the specified transaction. This variant lets a caller retry a job atomically alongside other database changes. A retried job isn't visible to be worked until the transaction commits, and if the transaction rolls back, so too is the retried job.

Jobs in the running state are not touched, while jobs in any other state are made available. To prevent jobs already waiting in the queue from being set back in line, the job's scheduled_at field is set to the current time only if it's not already in the past.

MaxAttempts is also incremented by one if the job has already exhausted its max attempts.

#### func (*Client[TTx]) [JobUpdate](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1533) ¶ added in v0.29.0

    func (c *Client[TTx]) JobUpdate(ctx [context](/context).[Context](/context#Context), id [int64](/builtin#int64), params *JobUpdateParams) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))

JobUpdate updates the job with the given ID.

If JobUpdateParams.Output is not set, this function may be used inside a job work function to set a job's output based on output recorded so far using RecordOutput.

#### func (*Client[TTx]) [JobUpdateTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1544) ¶ added in v0.29.0

    func (c *Client[TTx]) JobUpdateTx(ctx [context](/context).[Context](/context#Context), tx TTx, id [int64](/builtin#int64), params *JobUpdateParams) (*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), [error](/builtin#error))
...
#### func (*Client[TTx]) [Notify](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2383) ¶ added in v0.29.0

    func (c *Client[TTx]) Notify() *ClientNotifyBundle[TTx]

Notify retrieves a notification bundle for the client (in the sense of Postgres listen/notify) used to send notifications of various kinds.

#### func (*Client[TTx]) [PeriodicJobs](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2439) ¶ added in v0.2.0

    func (c *Client[TTx]) PeriodicJobs() *PeriodicJobBundle

PeriodicJobs returns the currently configured set of periodic jobs for the client, and can be used to add new or remove existing ones.

This function should only be invoked on clients capable of running perioidc jobs. Running periodic jobs requires that the client be electable as leader to run maintenance services, and being electable as leader requires that a client be started. To be startable, a client must have Queues and Workers configured. Invoking this function will panic if these conditions aren't met.

#### func (*Client[TTx]) [Pilot](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2451) ¶ added in v0.13.0

    func (c *Client[TTx]) Pilot() [riverpilot](/github.com/riverqueue/river/rivershared/riverpilot).[Pilot](/github.com/riverqueue/river/rivershared/riverpilot#Pilot)

Pilot returns the pilot in use by the pilot. If not configured, this is often simply StandardPilot.

API is not stable. DO NOT USE.

#### func (*Client[TTx]) [QueueGet](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2464) ¶ added in v0.5.0

    func (c *Client[TTx]) QueueGet(ctx [context](/context).[Context](/context#Context), name [string](/builtin#string)) (*[rivertype](/github.com/riverqueue/river/rivertype).[Queue](/github.com/riverqueue/river/rivertype#Queue), [error](/builtin#error))

QueueGet returns the queue with the given name. If the queue has not recently been active or does not exist, returns ErrNotFound.

The provided context is used for the underlying Postgres query and can be used to cancel the operation or apply a timeout.

#### func (*Client[TTx]) [QueueGetTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2476) ¶ added in v0.8.0

    func (c *Client[TTx]) QueueGetTx(ctx [context](/context).[Context](/context#Context), tx TTx, name [string](/builtin#string)) (*[rivertype](/github.com/riverqueue/river/rivertype).[Queue](/github.com/riverqueue/river/rivertype#Queue), [error](/builtin#error))

QueueGetTx returns the queue with the given name. If the queue has not recently been active or does not exist, returns ErrNotFound.

The provided context is used for the underlying Postgres query and can be used to cancel the operation or apply a timeout.

#### func (*Client[TTx]) [QueueList](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2501) ¶ added in v0.5.0

    func (c *Client[TTx]) QueueList(ctx [context](/context).[Context](/context#Context), params *QueueListParams) (*QueueListResult, [error](/builtin#error))

QueueList returns a list of all queues that are currently active or were recently active. Limit and offset can be used to paginate the results.
...
#### func (*Client[TTx]) [QueueListTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2528) ¶ added in v0.8.0

    func (c *Client[TTx]) QueueListTx(ctx [context](/context).[Context](/context#Context), tx TTx, params *QueueListParams) (*QueueListResult, [error](/builtin#error))

QueueListTx returns a list of all queues that are currently active or were recently active. Limit and offset can be used to paginate the results.
...
#### func (*Client[TTx]) [QueuePause](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2555) ¶ added in v0.5.0

    func (c *Client[TTx]) QueuePause(ctx [context](/context).[Context](/context#Context), name [string](/builtin#string), opts *QueuePauseOpts) [error](/builtin#error)

QueuePause pauses the queue with the given name. When a queue is paused, clients will not fetch any more jobs for that particular queue. To pause all queues at once, use the special queue name "*".

Clients with a configured notifier should receive a notification about the paused queue(s) within a few milliseconds of the transaction commit. Clients in poll-only mode will pause after their next poll for queue configuration.

The provided context is used for the underlying Postgres update and can be used to cancel the operation or apply a timeout. The opts are reserved for future functionality.

#### func (*Client[TTx]) [QueuePauseTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2595) ¶ added in v0.8.0

    func (c *Client[TTx]) QueuePauseTx(ctx [context](/context).[Context](/context#Context), tx TTx, name [string](/builtin#string), opts *QueuePauseOpts) [error](/builtin#error)

QueuePauseTx pauses the queue with the given name. When a queue is paused, clients will not fetch any more jobs for that particular queue. To pause all queues at once, use the special queue name "*".

Clients with a configured notifier should receive a notification about the paused queue(s) within a few milliseconds of the transaction commit. Clients in poll-only mode will pause after their next poll for queue configuration.

The provided context is used for the underlying Postgres update and can be used to cancel the operation or apply a timeout. The opts are reserved for future functionality.

#### func (*Client[TTx]) [QueueResume](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2625) ¶ added in v0.5.0

    func (c *Client[TTx]) QueueResume(ctx [context](/context).[Context](/context#Context), name [string](/builtin#string), opts *QueuePauseOpts) [error](/builtin#error)

QueueResume resumes the queue with the given name. If the queue was previously paused, any clients configured to work that queue will resume fetching additional jobs. To resume all queues at once, use the special queue name "*".

Clients with a configured notifier should receive a notification about the resumed queue(s) within a few milliseconds of the transaction commit. Clients in poll-only mode will resume after their next poll for queue configuration.

The provided context is used for the underlying Postgres update and can be used to cancel the operation or apply a timeout. The opts are reserved for future functionality.

#### func (*Client[TTx]) [QueueResumeTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2666) ¶ added in v0.8.0

    func (c *Client[TTx]) QueueResumeTx(ctx [context](/context).[Context](/context#Context), tx TTx, name [string](/builtin#string), opts *QueuePauseOpts) [error](/builtin#error)

QueueResumeTx resumes the queue with the given name. If the queue was previously paused, any clients configured to work that queue will resume fetching additional jobs. To resume all queues at once, use the special queue name "*".

Clients with a configured notifier should receive a notification about the resumed queue(s) within a few milliseconds of the transaction commit. Clients in poll-only mode will resume after their next poll for queue configuration.

The provided context is used for the underlying Postgres update and can be used to cancel the operation or apply a timeout. The opts are reserved for future functionality.

#### func (*Client[TTx]) [QueueUpdate](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2693) ¶ added in v0.20.0

    func (c *Client[TTx]) QueueUpdate(ctx [context](/context).[Context](/context#Context), name [string](/builtin#string), params *QueueUpdateParams) (*[rivertype](/github.com/riverqueue/river/rivertype).[Queue](/github.com/riverqueue/river/rivertype#Queue), [error](/builtin#error))

QueueUpdate updates a queue's settings in the database. These settings override the settings in the client (if applied).

#### func (*Client[TTx]) [QueueUpdateTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2716) ¶ added in v0.20.2

    func (c *Client[TTx]) QueueUpdateTx(ctx [context](/context).[Context](/context#Context), tx TTx, name [string](/builtin#string), params *QueueUpdateParams) (*[rivertype](/github.com/riverqueue/river/rivertype).[Queue](/github.com/riverqueue/river/rivertype#Queue), [error](/builtin#error))

QueueUpdateTx updates a queue's settings in the database. These settings override the settings in the client (if applied).

#### func (*Client[TTx]) [Queues](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2457) ¶ added in v0.10.0

    func (c *Client[TTx]) Queues() *QueueBundle

Queues returns the currently configured set of queues for the client, and can be used to add new ones.

#### func (*Client[TTx]) [Schema](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2783) ¶ added in v0.24.0

    func (c *Client[TTx]) Schema() [string](/builtin#string)

Schema returns the configured schema for the client.

#### func (*Client[TTx]) [Start](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L978) ¶

    func (c *Client[TTx]) Start(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)
...
#### func (*Client[TTx]) [Stop](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1145) ¶

    func (c *Client[TTx]) Stop(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)
...
#### func (*Client[TTx]) [StopAndCancel](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1171) ¶

    func (c *Client[TTx]) StopAndCancel(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)
...
#### func (*Client[TTx]) [Stopped](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1194) ¶ added in v0.0.11
...
#### func (*Client[TTx]) [Subscribe](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1215) ¶
...
#### func (*Client[TTx]) [SubscribeConfig](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1243) ¶ added in v0.1.0

    func (c *Client[TTx]) SubscribeConfig(config *SubscribeConfig) (<-chan *Event, func())

SubscribeConfig is a special internal variant of Subscribe that lets us inject an overridden channel size.

#### type [ClientNotifyBundle](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2390) ¶ added in v0.29.0

    type ClientNotifyBundle[TTx [any](/builtin#any)] struct {
     // contains filtered or unexported fields
    }

ClientNotifyBundle sends various notifications for a client (in the sense of Postgres listen/notify). Functions are on this bundle struct instead of the top-level client to keep them grouped together and better organized.

#### func (*ClientNotifyBundle[TTx]) [RequestResign](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2398) ¶ added in v0.29.0

    func (c *ClientNotifyBundle[TTx]) RequestResign(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)

RequestResign sends a notification requesting that the current leader resign. This usually causes the resignation of the current leader, but may have no effect if no leader is currently elected.

#### func (*ClientNotifyBundle[TTx]) [RequestResignTx](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2410) ¶ added in v0.29.0

    func (c *ClientNotifyBundle[TTx]) RequestResignTx(ctx [context](/context).[Context](/context#Context), tx TTx) [error](/builtin#error)

RequestResignTx sends a notification requesting that the current leader resign. This usually causes the resignation of the current leader, but may have no effect if no leader is currently elected.

This variant sends a notification in a transaction, which means that no notification is sent until the transaction commits.

#### type [ClientRetryPolicy](https://github.com/riverqueue/river/blob/v0.30.2/retry_policy.go#L20) ¶
    ...
    type ClientRetryPolicy interface {
     ...
     // consider.
     NextRetry(job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) [time](/time).[Time](/time#Time)
    }
...
#### type [Config](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L80) ¶
    ...
    type Config struct {
     ...
     AdvisoryLockPrefix [int32](/builtin#int32)
     ...
     CancelledJobRetentionPeriod [time](/time).[Duration](/time#Duration)
     ...
     //
     // Defaults to 24 hours.
     CompletedJobRetentionPeriod [time](/time).[Duration](/time#Duration)
     ...
     //
     // Defaults to 7 days.
     DiscardedJobRetentionPeriod [time](/time).[Duration](/time#Duration)
     ...
     //
     // Defaults to 100 ms.
     FetchCooldown [time](/time).[Duration](/time#Duration)
     ...
     FetchPollInterval [time](/time).[Duration](/time#Duration)
     ...
     // deployments.
     JobCleanerTimeout [time](/time).[Duration](/time#Duration)
     ...
     JobInsertMiddleware [][rivertype](/github.com/riverqueue/river/rivertype).[JobInsertMiddleware](/github.com/riverqueue/river/rivertype#JobInsertMiddleware)
     ...
     JobTimeout [time](/time).[Duration](/time#Duration)
     ...
     Hooks [][rivertype](/github.com/riverqueue/river/rivertype).[Hook](/github.com/riverqueue/river/rivertype#Hook)
     ...
     // or higher.
     Logger *[slog](/log/slog).[Logger](/log/slog#Logger)
     ...
     Middleware [][rivertype](/github.com/riverqueue/river/rivertype).[Middleware](/github.com/riverqueue/river/rivertype#Middleware)
     ...
     Queues map[[string](/builtin#string)]QueueConfig
     ...
     //
     // Defaults to 1 minute.
     ReindexerTimeout [time](/time).[Duration](/time#Duration)
     ...
     RescueStuckJobsAfter [time](/time).[Duration](/time#Duration)
     ...
     SkipJobKindValidation [bool](/builtin#bool)
     ...
     //
     // Defaults to false.
     SkipUnknownJobCheck [bool](/builtin#bool)
     ...
     WorkerMiddleware [][rivertype](/github.com/riverqueue/river/rivertype).[WorkerMiddleware](/github.com/riverqueue/river/rivertype#WorkerMiddleware)
     // contains filtered or unexported fields
    }

Config is the configuration for a Client.

Both Queues and Workers are required for a client to work jobs, but an insert-only client can be initialized by omitting Queues, and not calling Start for the client. Workers can also be omitted, but it's better to include it so River can check that inserted job kinds have a worker that can run them.

#### func (*Config) [WithDefaults](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L379) ¶ added in v0.17.0

    func (c *Config) WithDefaults() *Config

WithDefaults returns a copy of the Config with all default values applied.

#### type [DefaultClientRetryPolicy](https://github.com/riverqueue/river/blob/v0.30.2/retry_policy.go#L29) ¶
...
#### func (*DefaultClientRetryPolicy) [NextRetry](https://github.com/riverqueue/river/blob/v0.30.2/retry_policy.go#L49) ¶

    func (p *DefaultClientRetryPolicy) NextRetry(job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) [time](/time).[Time](/time#Time)
...
Snoozes do not count as attempts and do not influence retry behavior. Earlier versions of River would allow the attempt to increment each time a job was snoozed. Although this has been changed and snoozes now decrement the attempt count, we can maintain the same retry schedule even for pre-existing jobs by using the number of errors instead of the attempt count. This ensures consistent behavior across River versions.

At degenerately high retry counts (>= 310) the policy starts adding the equivalent of the maximum of time.Duration to each retry, about 292 years. The schedule is no longer exponential past this point.

#### type [ErrorHandler](https://github.com/riverqueue/river/blob/v0.30.2/error_handler.go#L12) ¶
    ...
    type ErrorHandler interface {
     ...
     HandleError(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), err [error](/builtin#error)) *ErrorHandlerResult
     ...
     // is available).
     HandlePanic(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), panicVal [any](/builtin#any), trace [string](/builtin#string)) *ErrorHandlerResult
    }
...
#### type [ErrorHandlerResult](https://github.com/riverqueue/river/blob/v0.30.2/error_handler.go#L30) ¶
    ...
    type ErrorHandlerResult struct {
     ...
     // schedule.
     SetCancelled [bool](/builtin#bool)
    }

#### type [Event](https://github.com/riverqueue/river/blob/v0.30.2/event.go#L50) ¶
    ...
    type Event struct {
     ...
     Job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)
    
     // JobStats are statistics about the run of a job.
     JobStats *JobStatistics
    
     // Queue contains queue-related information.
     Queue *[rivertype](/github.com/riverqueue/river/rivertype).[Queue](/github.com/riverqueue/river/rivertype#Queue)
    }

Event wraps an event that occurred within a River client, like a job being completed.

#### type [EventKind](https://github.com/riverqueue/river/blob/v0.30.2/event.go#L11) ¶

    type EventKind [string](/builtin#string)
...
#### type [HookDefaults](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L12) ¶ added in v0.19.0

    type HookDefaults struct{}

HookDefaults should be embedded on any hooks implementation. It helps identify a struct as hooks, and guarantee forward compatibility in case additions are necessary to the rivertype.Hook interface.

#### func (*HookDefaults) [IsHook](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L14) ¶ added in v0.19.0

    func (d *HookDefaults) IsHook() [bool](/builtin#bool)

#### type [HookInsertBeginFunc](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L18) ¶ added in v0.19.0

    type HookInsertBeginFunc func(ctx [context](/context).[Context](/context#Context), params *[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertParams](/github.com/riverqueue/river/rivertype#JobInsertParams)) [error](/builtin#error)

HookInsertBeginFunc is a convenience helper for implementing rivertype.HookInsertBegin using a simple function instead of a struct.

#### func (HookInsertBeginFunc) [InsertBegin](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L20) ¶ added in v0.19.0

    func (f HookInsertBeginFunc) InsertBegin(ctx [context](/context).[Context](/context#Context), params *[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertParams](/github.com/riverqueue/river/rivertype#JobInsertParams)) [error](/builtin#error)

#### func (HookInsertBeginFunc) [IsHook](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L24) ¶ added in v0.19.0

    func (f HookInsertBeginFunc) IsHook() [bool](/builtin#bool)

#### type [HookPeriodicJobsStartFunc](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L28) ¶ added in v0.29.0

    type HookPeriodicJobsStartFunc func(ctx [context](/context).[Context](/context#Context), params *[rivertype](/github.com/riverqueue/river/rivertype).[HookPeriodicJobsStartParams](/github.com/riverqueue/river/rivertype#HookPeriodicJobsStartParams)) [error](/builtin#error)

HookPeriodicJobsStartFunc is a convenience helper for implementing rivertype.HookPeriodicJobsStart using a simple function instead of a struct.

#### func (HookPeriodicJobsStartFunc) [IsHook](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L30) ¶ added in v0.29.0

    func (f HookPeriodicJobsStartFunc) IsHook() [bool](/builtin#bool)

#### func (HookPeriodicJobsStartFunc) [Start](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L32) ¶ added in v0.29.0

    func (f HookPeriodicJobsStartFunc) Start(ctx [context](/context).[Context](/context#Context), params *[rivertype](/github.com/riverqueue/river/rivertype).[HookPeriodicJobsStartParams](/github.com/riverqueue/river/rivertype#HookPeriodicJobsStartParams)) [error](/builtin#error)

#### type [HookWorkBeginFunc](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L38) ¶ added in v0.19.0

    type HookWorkBeginFunc func(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) [error](/builtin#error)

HookWorkBeginFunc is a convenience helper for implementing rivertype.HookWorkBegin using a simple function instead of a struct.

#### func (HookWorkBeginFunc) [IsHook](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L44) ¶ added in v0.19.0

    func (f HookWorkBeginFunc) IsHook() [bool](/builtin#bool)

#### func (HookWorkBeginFunc) [WorkBegin](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L40) ¶ added in v0.19.0

    func (f HookWorkBeginFunc) WorkBegin(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) [error](/builtin#error)

#### type [HookWorkEndFunc](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L48) ¶ added in v0.21.0

    type HookWorkEndFunc func(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), err [error](/builtin#error)) [error](/builtin#error)

HookWorkEndFunc is a convenience helper for implementing rivertype.HookWorkEnd using a simple function instead of a struct.

#### func (HookWorkEndFunc) [IsHook](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L54) ¶ added in v0.21.0

    func (f HookWorkEndFunc) IsHook() [bool](/builtin#bool)

#### func (HookWorkEndFunc) [WorkEnd](https://github.com/riverqueue/river/blob/v0.30.2/hook_defaults_funcs.go#L50) ¶ added in v0.21.0

    func (f HookWorkEndFunc) WorkEnd(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), err [error](/builtin#error)) [error](/builtin#error)

#### type [InsertManyParams](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1778) ¶
...
#### type [InsertOpts](https://github.com/riverqueue/river/blob/v0.30.2/insert_opts.go#L25) ¶
    ...
    type InsertOpts struct {
     ...
     ScheduledAt [time](/time).[Time](/time#Time)
...
#### type [Job](https://github.com/riverqueue/river/blob/v0.30.2/job.go#L9) ¶

    type Job[T JobArgs] struct {
     *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)
...
#### func [JobCompleteTx](https://github.com/riverqueue/river/blob/v0.30.2/job_complete_tx.go#L28) ¶

    func JobCompleteTx[TDriver [riverdriver](/github.com/riverqueue/river/riverdriver).[Driver](/github.com/riverqueue/river/riverdriver#Driver)[TTx], TTx [any](/builtin#any), TArgs JobArgs](ctx [context](/context).[Context](/context#Context), tx TTx, job *Job[TArgs]) (*Job[TArgs], [error](/builtin#error))
...
#### type [JobArgs](https://github.com/riverqueue/river/blob/v0.30.2/job.go#L21) ¶
...
#### type [JobArgsWithHooks](https://github.com/riverqueue/river/blob/v0.30.2/job.go#L75) ¶ added in v0.19.0
...
JobArgsWithHooks is an interface that job args can implement to attach specific hooks (i.e. other than those globally installed to a client) to certain kinds of jobs.

#### type [JobArgsWithInsertOpts](https://github.com/riverqueue/river/blob/v0.30.2/job.go#L90) ¶
...
#### type [JobArgsWithKindAliases](https://github.com/riverqueue/river/blob/v0.30.2/job.go#L66) ¶ added in v0.22.0

    type JobArgsWithKindAliases interface {
     // KindAliases returns alias kinds that an associated job args worker will
     // respond to.
     KindAliases() [][string](/builtin#string)
    }

JobArgsWithKindAliases is an interface that jobs args can implement to provide an alternate kind which a worker will be registered under in addition to the primary kind. This is useful for renaming a job kind in a safe manner so that any jobs already in the database aren't orphaned.

Renaming a job is a three part process. To begin, a job args with its original name:
...
Rename by putting the new name in Kind and moving the old name to KindAliases:
...
After all jobs inserted under the original name have finished working (including all their possible retries, which notably might take up to three weeks on the default retry policy), remove KindAliases:
...
#### type [JobCancelError](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L16) ¶ added in v0.14.0

    type JobCancelError = [rivertype](/github.com/riverqueue/river/rivertype).[JobCancelError](/github.com/riverqueue/river/rivertype#JobCancelError)
...
#### type [JobDeleteManyParams](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L13) ¶ added in v0.24.0

    type JobDeleteManyParams struct {
     // contains filtered or unexported fields
    }

JobDeleteManyParams specifies the parameters for a JobDeleteMany query. It must be initialized with NewJobDeleteManyParams. Params can be built by chaining methods on the JobDeleteManyParams object:

    params := NewJobDeleteManyParams().First(100).States(river.JobStateCompleted)
    

#### func [NewJobDeleteManyParams](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L26) ¶ added in v0.24.0
...
#### func (*JobDeleteManyParams) [First](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L70) ¶ added in v0.24.0

    func (p *JobDeleteManyParams) First(count [int](/builtin#int)) *JobDeleteManyParams

First returns an updated filter set that will only delete the first count jobs.

Count must be between 1 and 10_000, inclusive, or this will panic.

#### func (*JobDeleteManyParams) [IDs](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L84) ¶ added in v0.24.0

    func (p *JobDeleteManyParams) IDs(ids ...[int64](/builtin#int64)) *JobDeleteManyParams

IDs returns an updated filter set that will only delete jobs with the given IDs.

#### func (*JobDeleteManyParams) [Kinds](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L93) ¶ added in v0.24.0

    func (p *JobDeleteManyParams) Kinds(kinds ...[string](/builtin#string)) *JobDeleteManyParams

Kinds returns an updated filter set that will only delete jobs of the given kinds.

#### func (*JobDeleteManyParams) [Priorities](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L102) ¶ added in v0.24.0

    func (p *JobDeleteManyParams) Priorities(priorities ...[int16](/builtin#int16)) *JobDeleteManyParams

Priorities returns an updated filter set that will only delete jobs with the given priorities.

#### func (*JobDeleteManyParams) [Queues](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L111) ¶ added in v0.24.0

    func (p *JobDeleteManyParams) Queues(queues ...[string](/builtin#string)) *JobDeleteManyParams

Queues returns an updated filter set that will only delete jobs from the given queues.

#### func (*JobDeleteManyParams) [States](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L120) ¶ added in v0.24.0

    func (p *JobDeleteManyParams) States(states ...[rivertype](/github.com/riverqueue/river/rivertype).[JobState](/github.com/riverqueue/river/rivertype#JobState)) *JobDeleteManyParams

States returns an updated filter set that will only delete jobs in the given states.

#### func (*JobDeleteManyParams) [UnsafeAll](https://github.com/riverqueue/river/blob/v0.30.2/delete_many_params.go#L142) ¶ added in v0.25.0

    func (p *JobDeleteManyParams) UnsafeAll() *JobDeleteManyParams

UnsafeAll is a special directive that allows unbounded job deletion without any filters. Normally, filters like IDs or Kinds is required to scope down the deletion so that the caller doesn't accidentally delete all non-running jobs. Invoking UnsafeAll removes this safety guard so that all jobs can be removed arbitrarily.
...
It only makes sense to call this function if no filters have yet been applied on the parameters object. If some have already, calling it will panic.

#### type [JobDeleteManyResult](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2225) ¶ added in v0.24.0

    type JobDeleteManyResult struct {
     // Jobs is a slice of job returned as part of the list operation.
     Jobs []*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)
    }

JobDeleteManyResult is the result of a job list operation. It contains a list of jobs and a cursor for fetching the next page of results.

#### type [JobInsertMiddlewareDefaults](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L22) deprecated added in v0.13.0

    type JobInsertMiddlewareDefaults struct{ MiddlewareDefaults }

JobInsertMiddlewareDefaults is an embeddable struct that provides default implementations for the rivertype.JobInsertMiddleware. Use of this struct is recommended in case rivertype.JobInsertMiddleware is expanded in the future so that existing code isn't unexpectedly broken during an upgrade.

Deprecated: Prefer embedding the more general MiddlewareDefaults instead.

#### func (*JobInsertMiddlewareDefaults) [InsertMany](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L24) ¶ added in v0.13.0

    func (d *JobInsertMiddlewareDefaults) InsertMany(ctx [context](/context).[Context](/context#Context), manyParams []*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertParams](/github.com/riverqueue/river/rivertype#JobInsertParams), doInner func(ctx [context](/context).[Context](/context#Context)) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))

#### type [JobInsertMiddlewareFunc](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L30) ¶ added in v0.21.0

    type JobInsertMiddlewareFunc func(ctx [context](/context).[Context](/context#Context), manyParams []*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertParams](/github.com/riverqueue/river/rivertype#JobInsertParams), doInner func(ctx [context](/context).[Context](/context#Context)) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))

JobInsertMiddlewareFunc is a convenience helper for implementing rivertype.JobInsertMiddleware using a simple function instead of a struct.

#### func (JobInsertMiddlewareFunc) [InsertMany](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L32) ¶ added in v0.21.0

    func (f JobInsertMiddlewareFunc) InsertMany(ctx [context](/context).[Context](/context#Context), manyParams []*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertParams](/github.com/riverqueue/river/rivertype#JobInsertParams), doInner func(ctx [context](/context).[Context](/context#Context)) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))) ([]*[rivertype](/github.com/riverqueue/river/rivertype).[JobInsertResult](/github.com/riverqueue/river/rivertype#JobInsertResult), [error](/builtin#error))

#### func (JobInsertMiddlewareFunc) [IsMiddleware](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L36) ¶ added in v0.21.0

    func (f JobInsertMiddlewareFunc) IsMiddleware() [bool](/builtin#bool)

#### type [JobListCursor](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L18) ¶ added in v0.0.17
...
#### func [JobListCursorFromJob](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L28) ¶ added in v0.0.17

    func JobListCursorFromJob(job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) *JobListCursor

JobListCursorFromJob creates a JobListCursor from a JobRow.

#### func (JobListCursor) [MarshalText](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L91) ¶ added in v0.0.17

    func (c JobListCursor) MarshalText() ([][byte](/builtin#byte), [error](/builtin#error))

MarshalText implements encoding.TextMarshaler to encode the cursor as an opaque string.

#### func (*JobListCursor) [UnmarshalText](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L67) ¶ added in v0.0.17

    func (c *JobListCursor) UnmarshalText(text [][byte](/builtin#byte)) [error](/builtin#error)

UnmarshalText implements encoding.TextUnmarshaler to decode the cursor from a previously marshaled string.

#### type [JobListOrderByField](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L132) ¶ added in v0.0.17

    type JobListOrderByField [string](/builtin#string)
...
#### type [JobListParams](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L167) ¶ added in v0.0.17
...
#### func [NewJobListParams](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L185) ¶ added in v0.0.17
...
#### func (*JobListParams) [After](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L300) ¶ added in v0.0.17

    func (p *JobListParams) After(cursor *JobListCursor) *JobListParams

After returns an updated filter set that will only return jobs after the given cursor.

#### func (*JobListParams) [First](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L314) ¶ added in v0.0.17

    func (p *JobListParams) First(count [int](/builtin#int)) *JobListParams

First returns an updated filter set that will only return the first count jobs.

Count must be between 1 and 10_000, inclusive, or this will panic.

#### func (*JobListParams) [IDs](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L328) ¶ added in v0.21.0

    func (p *JobListParams) IDs(ids ...[int64](/builtin#int64)) *JobListParams

IDs returns an updated filter set that will only return jobs with the given IDs.

#### func (*JobListParams) [Kinds](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L337) ¶ added in v0.0.23

    func (p *JobListParams) Kinds(kinds ...[string](/builtin#string)) *JobListParams

Kinds returns an updated filter set that will only return jobs of the given kinds.

#### func (*JobListParams) [Metadata](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L354) ¶ added in v0.0.17

    func (p *JobListParams) Metadata(json [string](/builtin#string)) *JobListParams

Metadata returns an updated filter set that will return only jobs that has metadata which contains the given JSON fragment at its top level. This is equivalent to the `@>` operator in Postgres:

<https://www.postgresql.org/docs/current/functions-json.html>

This function isn't supported in SQLite due to SQLite not having an equivalent operator to use, so there's no efficient way to implement it. We recommend the use of Where using a condition with a comparison on the `->>` operator instead.

#### func (*JobListParams) [OrderBy](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L369) ¶ added in v0.0.17

    func (p *JobListParams) OrderBy(field JobListOrderByField, direction SortOrder) *JobListParams

OrderBy returns an updated filter set that will sort the results using the specified field and direction.

If ordering by FinalizedAt, the States filter will be set to only include finalized job states unless it has already been overridden.

#### func (*JobListParams) [Priorities](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L393) ¶ added in v0.21.0

    func (p *JobListParams) Priorities(priorities ...[int16](/builtin#int16)) *JobListParams

Priorities returns an updated filter set that will only return jobs with the given priorities.

#### func (*JobListParams) [Queues](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L402) ¶ added in v0.0.17

    func (p *JobListParams) Queues(queues ...[string](/builtin#string)) *JobListParams

Queues returns an updated filter set that will only return jobs from the given queues.

#### func (*JobListParams) [States](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L411) ¶ added in v0.4.0

    func (p *JobListParams) States(states ...[rivertype](/github.com/riverqueue/river/rivertype).[JobState](/github.com/riverqueue/river/rivertype#JobState)) *JobListParams

States returns an updated filter set that will only return jobs in the given states.

#### func (*JobListParams) [Where](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L452) ¶ added in v0.23.0

    func (p *JobListParams) Where(sql [string](/builtin#string), namedArgsMany ...NamedArgs) *JobListParams

Where is an all-encompassing query escape hatch that adds an arbitrary predicate after a list query's `WHERE ...` clause. Use of other JobListParams filters should be preferred where possible because they're safer and their compatibility between drivers is better guaranteed, but in case none is suitable, Where can be used as a last resort.

For example, using Where to query with `jsonb_path_query_first(...)` using a JSON path, a function that's specific to Postgres:

    listParams = listParams.Where("jsonb_path_query_first(metadata, @json_path) = @json_val", NamedArgs{"json_path": "$.foo", "json_val": `"bar"`})
    

A JSON path can be used in a query in SQLite as well, but there the `->` or `->>` operators must be used instead:

    listParams = listParams.Where("metadata ->> @json_path = @json_val", NamedArgs{"json_path": "$.foo", "json_val": "bar"})
    

Arguments beyond the first are interpreted as named parameters. Each one should be present in the query SQL prefixed with a `@` symbol. Multiple sets of named parameters will be merged together, with values in later sets overwriting those in earlier ones.

Calling Where multiple times will add multiple conditions separate by `AND`. Use `OR` instead by stuffing all conditions into a single Where invocation.

Consider use of this function possibly hazardous! Any time raw SQL is in play, an application is opening itself up to SQL injection attacks. Never mix unsanitized user input into a SQL string, and use named parameters to curb the likelihood of injection.

#### type [JobListResult](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2283) ¶ added in v0.4.0
...
JobListResult is the result of a job list operation. It contains a list of jobs and a cursor for fetching the next page of results.

#### type [JobSnoozeError](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L28) ¶ added in v0.14.0

    type JobSnoozeError = [rivertype](/github.com/riverqueue/river/rivertype).[JobSnoozeError](/github.com/riverqueue/river/rivertype#JobSnoozeError)
...
#### type [JobStatistics](https://github.com/riverqueue/river/blob/v0.30.2/event.go#L67) ¶

    type JobStatistics struct {
     CompleteDuration  [time](/time).[Duration](/time#Duration) // Time it took to set the job completed, discarded, or errored.
     QueueWaitDuration [time](/time).[Duration](/time#Duration) // Time the job spent waiting in available state before starting execution.
     RunDuration       [time](/time).[Duration](/time#Duration) // Time job spent running (measured around job worker.)
    }

JobStatistics contains information about a single execution of a job.

#### type [JobUpdateParams](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1520) ¶ added in v0.29.0
    ...
    type JobUpdateParams struct {
     ...
     Output [any](/builtin#any)
    }

JobUpdateParams contains parameters for Client.JobUpdate and Client.JobUpdateTx.

#### type [MiddlewareDefaults](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L12) ¶ added in v0.19.0

    type MiddlewareDefaults struct{}

MiddlewareDefaults should be embedded on any middleware implementation. It helps identify a struct as middleware, and guarantees forward compatibility in case additions are necessary to the rivertype.Middleware interface.

#### func (*MiddlewareDefaults) [IsMiddleware](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L14) ¶ added in v0.19.0

    func (d *MiddlewareDefaults) IsMiddleware() [bool](/builtin#bool)

#### type [NamedArgs](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L422) ¶ added in v0.23.0

    type NamedArgs map[[string](/builtin#string)][any](/builtin#any)

NamedArgs are named arguments for use with JobListParams.Where. Keys should look like "my_param", and map to parameters like "@my_param" in SQL queries. "@" are present in the SQL, but not in the keys of this map.

#### type [PeriodicJob](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L29) ¶
...
#### func [NewPeriodicJob](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L74) ¶
...
#### type [PeriodicJobBundle](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L111) ¶ added in v0.2.0

    type PeriodicJobBundle struct {
     // contains filtered or unexported fields
    }

PeriodicJobBundle is a bundle of currently configured periodic jobs. It's made accessible through Client, where new periodic jobs can be configured, and old ones removed.

#### func (*PeriodicJobBundle) [Add](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L133) ¶ added in v0.2.0

    func (b *PeriodicJobBundle) Add(periodicJob *PeriodicJob) [rivertype](/github.com/riverqueue/river/rivertype).[PeriodicJobHandle](/github.com/riverqueue/river/rivertype#PeriodicJobHandle)

Add adds a new periodic job to the client. The job is queued immediately if RunOnStart is enabled, and then scheduled normally.

Returns a periodic job handle which can be used to subsequently remove the job if desired.

Adding or removing periodic jobs has no effect unless this client is elected leader because only the leader enqueues periodic jobs. To make sure that a new periodic job is fully enabled or disabled, it should be added or removed from _every_ active River client across all processes.

#### func (*PeriodicJobBundle) [AddMany](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L157) ¶ added in v0.2.0

    func (b *PeriodicJobBundle) AddMany(periodicJobs []*PeriodicJob) [][rivertype](/github.com/riverqueue/river/rivertype).[PeriodicJobHandle](/github.com/riverqueue/river/rivertype#PeriodicJobHandle)

AddMany adds many new periodic jobs to the client. The jobs are queued immediately if their RunOnStart is enabled, and then scheduled normally.

Returns a periodic job handle which can be used to subsequently remove the job if desired.

Adding or removing periodic jobs has no effect unless this client is elected leader because only the leader enqueues periodic jobs. To make sure that a new periodic job is fully enabled or disabled, it should be added or removed from _every_ active River client across all processes.

#### func (*PeriodicJobBundle) [AddManySafely](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L167) ¶ added in v0.23.0

    func (b *PeriodicJobBundle) AddManySafely(periodicJobs []*PeriodicJob) ([][rivertype](/github.com/riverqueue/river/rivertype).[PeriodicJobHandle](/github.com/riverqueue/river/rivertype#PeriodicJobHandle), [error](/builtin#error))
...
#### func (*PeriodicJobBundle) [AddSafely](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L143) ¶ added in v0.23.0

    func (b *PeriodicJobBundle) AddSafely(periodicJob *PeriodicJob) ([rivertype](/github.com/riverqueue/river/rivertype).[PeriodicJobHandle](/github.com/riverqueue/river/rivertype#PeriodicJobHandle), [error](/builtin#error))

AddSafely is the same as Add, but it returns an error in the case of a validation problem or duplicate ID instead of panicking.

#### func (*PeriodicJobBundle) [Clear](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L177) ¶ added in v0.2.0
...
#### func (*PeriodicJobBundle) [Remove](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L190) ¶ added in v0.2.0

    func (b *PeriodicJobBundle) Remove(periodicJobHandle [rivertype](/github.com/riverqueue/river/rivertype).[PeriodicJobHandle](/github.com/riverqueue/river/rivertype#PeriodicJobHandle))

Remove removes a periodic job, cancelling all scheduled runs.

Requires the use of the periodic job handle that was returned when the job was added.

Adding or removing periodic jobs has no effect unless this client is elected leader because only the leader enqueues periodic jobs. To make sure that a new periodic job is fully enabled or disabled, it should be added or removed from _every_ active River client across all processes.

#### func (*PeriodicJobBundle) [RemoveByID](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L205) ¶ added in v0.27.0

    func (b *PeriodicJobBundle) RemoveByID(id [string](/builtin#string)) [bool](/builtin#bool)
...
Returns true if a job with the given ID existed (and was removed), and false otherwise.

#### func (*PeriodicJobBundle) [RemoveMany](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L218) ¶ added in v0.2.0

    func (b *PeriodicJobBundle) RemoveMany(periodicJobHandles [][rivertype](/github.com/riverqueue/river/rivertype).[PeriodicJobHandle](/github.com/riverqueue/river/rivertype#PeriodicJobHandle))

RemoveMany removes many periodic jobs, cancelling all scheduled runs.

Requires the use of the periodic job handles that were returned when the jobs were added.

Adding or removing periodic jobs has no effect unless this client is elected leader because only the leader enqueues periodic jobs. To make sure that a new periodic job is fully enabled or disabled, it should be added or removed from _every_ active River client across all processes.

#### func (*PeriodicJobBundle) [RemoveManyByID](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L231) ¶ added in v0.27.0

    func (b *PeriodicJobBundle) RemoveManyByID(ids [][string](/builtin#string))

RemoveManyByID removes many periodic jobs by ID, cancelling all scheduled runs.

Adding or removing periodic jobs has no effect unless this client is elected leader because only the leader enqueues periodic jobs. To make sure that a new periodic job is fully enabled or disabled, it should be added or removed from _every_ active River client across all processes.

Has no effect if no jobs with the given IDs are configured.

#### type [PeriodicJobConstructor](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L26) ¶
...
#### type [PeriodicJobOpts](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L36) ¶
    ...
    type PeriodicJobOpts struct {
     ...
     // normal run schedule.
     RunOnStart [bool](/builtin#bool)
    }

PeriodicJobOpts are options for a periodic job.

#### type [PeriodicSchedule](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L15) ¶

    type PeriodicSchedule interface {
     // Next returns the next time at which the job should be run given the
     // current time.
     Next(current [time](/time).[Time](/time#Time)) [time](/time).[Time](/time#Time)
    }
...
#### func [NeverSchedule](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L90) ¶ added in v0.16.0

    func NeverSchedule() PeriodicSchedule

NeverSchedule returns a PeriodicSchedule that never runs.

#### func [PeriodicInterval](https://github.com/riverqueue/river/blob/v0.30.2/periodic_job.go#L100) ¶

    func PeriodicInterval(interval [time](/time).[Duration](/time#Duration)) PeriodicSchedule

PeriodicInterval returns a simple PeriodicSchedule that runs at the given interval.

#### type [QueueAlreadyAddedError](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L49) ¶ added in v0.23.0

    type QueueAlreadyAddedError struct {
     Name [string](/builtin#string)
    }

QueueAlreadyAddedError is returned when attempting to add a queue that has already been added to the Client.

#### func (*QueueAlreadyAddedError) [Error](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L53) ¶ added in v0.23.0

    func (e *QueueAlreadyAddedError) Error() [string](/builtin#string)

#### func (*QueueAlreadyAddedError) [Is](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L57) ¶ added in v0.23.0

    func (e *QueueAlreadyAddedError) Is(target [error](/builtin#error)) [bool](/builtin#bool)

#### type [QueueBundle](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2787) ¶ added in v0.10.0
...
#### func (*QueueBundle) [Add](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2809) ¶ added in v0.10.0

    func (b *QueueBundle) Add(queueName [string](/builtin#string), queueConfig QueueConfig) [error](/builtin#error)
...
#### type [QueueConfig](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L529) ¶
    ...
    type QueueConfig struct {
     ...
     MaxWorkers [int](/builtin#int)
    }

QueueConfig contains queue-specific configuration.

#### type [QueueListParams](https://github.com/riverqueue/river/blob/v0.30.2/queue_list_params.go#L8) ¶ added in v0.5.0

    type QueueListParams struct {
     // contains filtered or unexported fields
    }

QueueListParams specifies the parameters for a QueueList query. It must be initialized with NewQueueListParams. Params can be built by chaining methods on the QueueListParams object:

    params := NewQueueListParams().First(100)
    

#### func [NewQueueListParams](https://github.com/riverqueue/river/blob/v0.30.2/queue_list_params.go#L14) ¶ added in v0.5.0
...
#### func (*QueueListParams) [First](https://github.com/riverqueue/river/blob/v0.30.2/queue_list_params.go#L30) ¶ added in v0.5.0

    func (p *QueueListParams) First(count [int](/builtin#int)) *QueueListParams

First returns an updated filter set that will only return the first count queues.

Count must be between 1 and 10000, inclusive, or this will panic.

#### type [QueueListResult](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2485) ¶ added in v0.5.0

    type QueueListResult struct {
     // Queues is a slice of queues returned as part of the list operation.
     Queues []*[rivertype](/github.com/riverqueue/river/rivertype).[Queue](/github.com/riverqueue/river/rivertype#Queue)
    }

QueueListResult is the result of a job list operation. It contains a list of jobs and leaves room for future cursor functionality.

#### type [QueuePauseOpts](https://github.com/riverqueue/river/blob/v0.30.2/queue_pause_opts.go#L4) ¶ added in v0.5.0

    type QueuePauseOpts struct{}

QueuePauseOpts are optional settings for pausing or resuming a queue.

#### type [QueueUpdateParams](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L2685) ¶ added in v0.20.0
...
#### type [SortOrder](https://github.com/riverqueue/river/blob/v0.30.2/job_list_params.go#L121) ¶ added in v0.0.17

    type SortOrder [int](/builtin#int)

SortOrder specifies the direction of a sort.

    const (
     // SortOrderAsc specifies that the sort should in ascending order.
     SortOrderAsc SortOrder = [iota](/builtin#iota)
    
     // SortOrderDesc specifies that the sort should in descending order.
     SortOrderDesc
    )

#### type [SubscribeConfig](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L1225) ¶ added in v0.1.0
...
#### type [TestConfig](https://github.com/riverqueue/river/blob/v0.30.2/client.go#L59) ¶ added in v0.17.0
    ...
    type TestConfig struct {
     ...
     // when creating jobs.
     DisableUniqueEnforcement [bool](/builtin#bool)
    
     // Time is a time generator to make time stubbable in tests.
     Time [rivertype](/github.com/riverqueue/river/rivertype).[TimeGenerator](/github.com/riverqueue/river/rivertype#TimeGenerator)
    }

TestConfig contains configuration specific to test environments.

#### type [UniqueOpts](https://github.com/riverqueue/river/blob/v0.30.2/insert_opts.go#L102) ¶
    ...
    type UniqueOpts struct {
     ...
     ByPeriod [time](/time).[Duration](/time#Duration)
     ...
     ByState [][rivertype](/github.com/riverqueue/river/rivertype).[JobState](/github.com/riverqueue/river/rivertype#JobState)
     ...
     ExcludeKind [bool](/builtin#bool)
    }

UniqueOpts contains parameters for uniqueness for a job.

When the options struct is uninitialized (its zero value) no uniqueness at is enforced. As each property is initialized, it's added as a dimension on the uniqueness matrix. When any property has a non-zero value specified, the job's kind automatically counts toward uniqueness, but can be excluded by setting ExcludeKind to true.
...
Uniqueness relies on a hash of the job kind and any unique properties along with a database unique constraint. See the note on ByState for more details including about the fallback to a deprecated advisory lock method.

#### type [UnknownJobKindError](https://github.com/riverqueue/river/blob/v0.30.2/error.go#L64) ¶

    type UnknownJobKindError = [rivertype](/github.com/riverqueue/river/rivertype).[UnknownJobKindError](/github.com/riverqueue/river/rivertype#UnknownJobKindError)
...
#### type [Worker](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L39) ¶

    type Worker[T JobArgs] interface {
     // Middleware returns the type-specific middleware for this job.
     Middleware(job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) [][rivertype](/github.com/riverqueue/river/rivertype).[WorkerMiddleware](/github.com/riverqueue/river/rivertype#WorkerMiddleware)
     ...
     NextRetry(job *Job[T]) [time](/time).[Time](/time#Time)
     ...
     Timeout(job *Job[T]) [time](/time).[Duration](/time#Duration)
...
#### func [WorkFunc](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L215) ¶

    func WorkFunc[T JobArgs](f func([context](/context).[Context](/context#Context), *Job[T]) [error](/builtin#error)) Worker[T]
...
#### type [WorkerDefaults](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L75) ¶
...
#### func (WorkerDefaults[T]) [Middleware](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L77) ¶ added in v0.13.0

    func (w WorkerDefaults[T]) Middleware(*[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow)) [][rivertype](/github.com/riverqueue/river/rivertype).[WorkerMiddleware](/github.com/riverqueue/river/rivertype#WorkerMiddleware)

#### func (WorkerDefaults[T]) [NextRetry](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L82) ¶

    func (w WorkerDefaults[T]) NextRetry(*Job[T]) [time](/time).[Time](/time#Time)
...
#### func (WorkerDefaults[T]) [Timeout](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L86) ¶

    func (w WorkerDefaults[T]) Timeout(*Job[T]) [time](/time).[Duration](/time#Duration)
...
#### type [WorkerMiddlewareDefaults](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L44) deprecated added in v0.13.0

    type WorkerMiddlewareDefaults struct{ MiddlewareDefaults }

WorkerInsertMiddlewareDefaults is an embeddable struct that provides default implementations for the rivertype.WorkerMiddleware. Use of this struct is recommended in case rivertype.WorkerMiddleware is expanded in the future so that existing code isn't unexpectedly broken during an upgrade.

Deprecated: Prefer embedding the more general MiddlewareDefaults instead.

#### func (*WorkerMiddlewareDefaults) [Work](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L46) ¶ added in v0.13.0

    func (d *WorkerMiddlewareDefaults) Work(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), doInner func(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)) [error](/builtin#error)

#### type [WorkerMiddlewareFunc](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L52) ¶ added in v0.21.0

    type WorkerMiddlewareFunc func(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), doInner func(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)) [error](/builtin#error)
...
#### func (WorkerMiddlewareFunc) [IsMiddleware](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L58) ¶ added in v0.21.0

    func (f WorkerMiddlewareFunc) IsMiddleware() [bool](/builtin#bool)

#### func (WorkerMiddlewareFunc) [Work](https://github.com/riverqueue/river/blob/v0.30.2/middleware_defaults.go#L54) ¶ added in v0.21.0

    func (f WorkerMiddlewareFunc) Work(ctx [context](/context).[Context](/context#Context), job *[rivertype](/github.com/riverqueue/river/rivertype).[JobRow](/github.com/riverqueue/river/rivertype#JobRow), doInner func(ctx [context](/context).[Context](/context#Context)) [error](/builtin#error)) [error](/builtin#error)

#### type [Workers](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L137) ¶
...
#### func [NewWorkers](https://github.com/riverqueue/river/blob/v0.30.2/worker.go#L152) ¶
