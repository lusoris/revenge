...
For a complete enumeration of the available configuration options, see [_Settings_](../settings/).
...
There are a few exceptions to these rules:
  ...
  2. If a configuration file is passed directly via `\--config`, those settings are used for _all_ analyzed files, and any relative paths in that configuration file (like `exclude` globs or `src` paths) are resolved relative to the _current_ working directory.
...
Paths provided to `include` _must_ match files. For example, `include = ["src"]` will fail since it matches a directory.
