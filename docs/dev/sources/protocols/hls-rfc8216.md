...
> Fetched: 2026-01-30T23:52:19.723311+00:00
> Content-Hash: e1ce98b70d16ed97
> Type: html
...
HTTP Live Streaming               August 2017
...
. Loading the Media Playlist File ....................

44

6.3.3

. Playing the Media Playlist File ....................

45

6.3.4

. Reloading the Media Playlist File ..................
...
. Live Media Playlist Using HTTPS ...........................

50

8.3

. Playlist with Encrypted Media Segments ....................
...
. Session Data in a Master Playlist .........................
...
. EXT-X-DATERANGE Carrying SCTE-35 Tags ....................
...
).  Unmarked media discontinuities can trigger
   playback errors.
...
).  The Media Initialization Section MUST NOT contain
   sample data.
...
].  Unlike regular MPEG-4 files that have a Movie Box
...
], including the constraints on Media Data Boxes
   in Section 8.16 of [
...
] files that also satisfy the constraints described in this
   section.
...
) MAY omit the portion of the Media Data Box
...
AC_3
   ...
   sample with an ID3 Private frame (PRIV) tag [
...
A WebVTT Segment is a section of a WebVTT [
...
.
   ...
   The format of the Playlist files is derived from the M3U [
...
Section 4.3.4.2
...
).  Each Media Segment is specified by a URI and the
   ...
   Segment divided by its EXTINF duration (
...
) that specifies the duration of the
...
for more information about the EXT-
   X-DISCONTINUITY tag.
...
] with a 128-bit key, Cipher Block Chaining (CBC), and
...
].  Encryption of other Media Segment
      formats containing H.264 [
...
] media streams is described in the HTTP
      Live Streaming (HLS) Sample Encryption specification [

SampleEnc

].
      The IV attribute MAY be present; see
...
for when the IV attribute is used.

      KEYFORMAT

      The value is a quoted-string that specifies how the key is
      represented in the resource identified by the URI; see
...
for the format of the Key file and Sections
...
The EXT-X-PROGRAM-DATE-TIME tag associates the first sample of a
   ...
   where date-time-msec is an ISO/IEC 8601:2004 [
...
for more information on the EXT-X-
...
.  Mapping SCTE-35 into EXT-X-DATERANGE
...
] MAY be represented in a Media Playlist using
...
The EXT-X-MEDIA-SEQUENCE tag indicates the Media Sequence Number of
   ...
   Playlists contain matching content (see
...
for more information on setting the EXT-
...
defines the implications of the EXT-X-PLAYLIST-TYPE
   tag.
   ...
   Playlist can be updated according to the rules in
...
Section 4.3.4.2.1
   ...
   The following attributes are defined:
      ...
      Typically, closed-caption [

CEA608

] media is carried in the video
...
], which identifies the primary
      language used in the Rendition.  This attribute is OPTIONAL.

      ASSOC-LANGUAGE

      The value is a quoted-string containing a language tag [
...
] separated by comma (,) characters.  This
...
A set of one or more EXT-X-MEDIA tags with the same GROUP-ID value
   ...
   o  Each EXT-X-MEDIA tag with an AUTOSELECT=YES attribute SHOULD have
      a combination of LANGUAGE [

RFC5646

], ASSOC-LANGUAGE, FORCED, and
...
Every EXT-X-STREAM-INF tag MUST include the BANDWIDTH attribute.
      ...
      Parameters for "Bucket" Media Types" [
...
] or equivalent.  A value of NONE indicates that the
...
.
      ...
      used when playing the presentation.  See
...
The EXT-X-SESSION-DATA tag allows arbitrary session data to be
   ...
   The following attributes are defined:
      ...
      identified by the URI MUST be formatted as JSON [
...
The EXT-X-SESSION-KEY tag allows encryption keys from Media Playlists
   ...
   All attributes defined for the EXT-X-KEY tag (
...
] encryption uses 16-octet keys.  If the KEYFORMAT of an EXT-
...
] REQUIRES the same 16-octet IV to be supplied when
...
The server SHOULD attempt to divide the source media at points that
   ...
   The server must create a Media Playlist file (
...
Section 4
   ...
   The value of the EXT-X-VERSION tag (
...
Section 6.2.2
   ...
   o  Increment the value of the EXT-X-MEDIA-SEQUENCE or EXT-X-
      DISCONTINUITY-SEQUENCE tags (

Section 6.2.2

).

   o  Add an EXT-X-ENDLIST tag to the Playlist (
...
The server MAY limit the availability of Media Segments by removing
   Media Segments from the Playlist file (
...
Media Segments MAY be encrypted.  Every encrypted Media Segment MUST
   have an EXT-X-KEY tag (

Section 4.3.2.4

) applied to it with a URI that
   the client can use to obtain a Key file (
...
SHALL be applied to individual Media Segments.
...
RFC5652
   ...
   attribute of the EXT-X-KEY tag, as described in
...
] present in any Media Segment in any of the
...
The server MUST meet the following constraints when producing Variant
   ...
   o  Matching content in Variant Streams MUST have matching
      Discontinuity Sequence Numbers (see
...
How the client obtains the URI to the Playlist file is outside the
   ...
   Clients MUST ensure that loaded Playlists comply with
...
, if it intends to play the presentation
...
to determine the approximate location of
   ...
   synchronize between streams (see
...
.  CHARACTERISTICS Attribute Containing Multiple Characteristics
...
Since the protocol generally uses HTTP to transfer data, most of the
   same security considerations apply.  See
...
] to protect themselves from attack.  See also the
...
RFC2964
   ...
   SHOULD be secured by a mechanism such as HTTP Over TLS [
...
]     Advanced Television Systems Committee, "Digital Audio
...
]   Consumer Electronics Association, "ANSI/CEA 608-E: Line 21
...
]   Consumer Technology Association, "Digital Television (DTV)
...
]     Digital Content Protection LLC, "High-bandwidth Digital
...
, DOI 10.17487/RFC2964, October 2000,
              <
...
]  Gellens, R., Singer, D., and P. Frojdh, "The 'Codecs' and
...
]   Society of Cable Telecommunications Engineers, "Digital
              Program Insertion Cueing Message for Cable", ANSI/SCTE 35,
              August 2014, <

http://www.scte.org/documents/pdf/Standards/
...
]      Nullsoft, Inc., "The M3U Playlist format, originally
...
]      Apple Inc., "Uniform Type Identifier",
              <
...
draft-pantos-http-live-streaming-22

draft-pantos-http-live-streaming-21

draft-pantos-http-live-streaming-20

draft-pantos-http-live-streaming-19

draft-pantos-http-live-streaming-18

draft-pantos-http-live-streaming-17

draft-pantos-http-live-streaming-16

draft-pantos-http-live-streaming-15
...
draft-pantos-http-live-streaming-05

draft-pantos-http-live-streaming-04

draft-pantos-http-live-streaming-03

draft-pantos-http-live-streaming-02
