...
> Fetched: 2026-01-30T23:52:31.449181+00:00
> Content-Hash: 669ae4c403e61eb1
> Type: html
...
(Web Video Text Tracks) format is intended for marking up external text
track resources in connection with the HTML <track> element.

WebVTT files provide captions or subtitles for video content, and also text video descriptions

[MAUR]

, chapters for content navigation, and more generally any form of metadata that is
...
a time-interval, called a cue (

definition

). Beyond captioning and subtitling,
...
with a HTML file through a <track> element, the
...
CSS style sheets that apply to an HTML page that contains a

video

element can
target WebVTT cues and regions in the video using the
...
In this example, an HTML page has a CSS style sheet in a
...
" cannot be used in the style sheet. If the style sheet is wrapped in
"
...
", then those strings can just be removed. If
"
...
can be represented as "\31 23" (31 refers to the Unicode code point for "1"). See

Using character escapes in markup
 and CSS

for more information on CSS escapes.
...
The cues cover only 35% of the video viewport’s width - that’s the

cue
 box

’s "size" for all three cues.

The first cue has its

cue box

positioned at the 10% mark. The
 "line-left" and "line-right" within the "position" setting indicates which side of the

cue box

the position refers to. Since in this case the text is horizontal,
 ...
 viewport’s height and the

cue box

would cover 35% of the video
 viewport’s height.
...
The second cue has its

cue box

right aligned at the 90% mark of the
...
defines what consists of a valid

WebVTT file

. Authors need to
follow the requirements therein, and are encouraged to use a conformance checker.

§6 Parsing

defines how user agents are to interpret a file labelled as
...
All processing requirements in this specification apply. The user agent must also be
...
All processing requirements in this specification apply, except those in
...
All processing requirements in this specification apply, except parts of

§6 Parsing

that
 relate to stylesheets and CSS, and all of
...
. The user agent
 must instead only render the text inside

WebVTT caption or subtitle cue text

in an
 appropriate manner and specifically support the color classes defined in

§5 Default classes for WebVTT Caption or Subtitle Cue Components

. Any
 other styling instructions are optional.

User agents that do not support a full HTML CSS engine

All processing requirements in this specification apply, including the color classes defined
 in

§5 Default classes for WebVTT Caption or Subtitle Cue Components

. However, the user agent will need to apply the CSS related features in
...
in such a way that the rendered results are
 equivalent to what a full CSS supporting renderer produces.

User agents that support a full HTML CSS engine

All processing requirements in this specification apply. However, only a limited set of CSS
 styles is allowed because
...
will need
 ...
 bleeding in extra CSS styles that are beyond the WebVTT specification.

Conformance checkers

Conformance checkers must verify that a

WebVTT file

conforms to the applicable
 conformance criteria described in this specification. The term "validator" is equivalent to
 conformance checker for the purpose of this specification.

Authoring tools

Authoring tools must generate conforming
...
are also considered to be authoring tools.
...
data kind of a WebVTT file is externally specified, such as in a HTML file’s

text track

element. The environment is responsible for interpreting the data correctly.
...
that additionally consist of the
following:
...
additional properties allowing the

cue text

to be rendered and converted to a DOM
fragment:
...
is a box within which the text of all lines of the cue is to
  be rendered. It is either rendered into the video’s viewport or a region inside the viewport if
  the cue is part of a region.

The position of the

cue box

within the video viewport’s
  or region’s dimensions depends on the value of the
...
if lines' lengths make this necessary.
...
cue settings to be interpreted with respect to either the
...
writing direction could be used for vertical Chinese, Japanese, and Korean, and the

vertical growing right

writing
  direction could be used for vertical Mongolian.
...
is an integer number of lines
  (using the line dimensions of the first line of the cue), or whether it is a percentage of the
  dimension of the video. The flag is set to true when lines are counted, and false otherwise.

Cues where the flag is false will be offset as requested modulo overlap avoidance if multiple
  cues are in the same place.
...
from the
  top, the right or left of the video viewport as defined by the
...
, or the lines occupied by any other showing tracks.

The

line

is set either as a number of lines, a percentage of the
  video viewport height or width, or as the special value

auto

, which means the offset is to depend on the other showing tracks.
...
percentages are relative to the height of the video, otherwise to the width of the video.
...
is negative or
    greater than 100, then return 100 and abort these steps.
...
to a number outside the range 0..100 and also set the

WebVTT cue snap-to-lines
    flag

to false, this can happen when using the DOM API’s
...
is numeric, return the value of the

WebVTT cue
   line

and abort these steps. (Either the

WebVTT cue snap-to-lines flag

is true, so any
   value, not just those in the range 0..100, is valid, or the value is in the range 0..100 and is
   thus valid regardless of the value of that flag.)
...
is false, return the
   value 100 and abort these steps. (The
...
, return −1 and abort these steps.
...
will be −1 and the second will be −2.
...
— right-to-left vs. left-to-right cue text
  does not affect the
...
is either a number giving the position of the

cue box

as a percentage value or the special value

auto

, which means the position is to depend on the

text alignment

of the cue.

If the cue is not within a

region

, the percentage value is to be
...
percentages are relative to the width of the video, otherwise to the height of the video.
...
is numeric between 0 and 100, then return
   the value of the

position

and abort these steps. (Otherwise, the
...
cues with
  right-to-left cue text, the

cue box

is positioned from the left edge of
  the video viewport. This allows defining a rendering space template which can be filled with
  either left-to-right or right-to-left cue text, or both.
...
, authors must not use the default
...
aligned text, where the

auto

position

is 0% and 100%, respectively. The above requirement is present because it
  can be surprising that automatic positioning doesn’t work for
...
can consist of text with left-to-right base direction, or right-to-left base direction,
  or both (on different lines), such automatic positioning would have unexpected results.
...
’s alignment depends on the value of the
...
whose value is that returned by the following algorithm, which is defined in terms
...
if the base direction of
   the cue text is left-to-right,
...
value varies between left and top for horizontal and vertical cues.
...
The text of each line is individually aligned towards the start side of the box, where the
   start side for that line is determined by using the CSS rules for
...
The text of each line is individually aligned towards the end side of the box, where the end
   side for that line is determined by using the CSS rules for
...
The text is aligned to the box’s left side (for

horizontal

cues) or top side (otherwise).

Right alignment

The text is aligned to the box’s right side (for

horizontal

cues) or bottom side (otherwise).
...
The base direction of each line in a cue (which is used by the Unicode Bidirectional
...
algorithm. In the occasional cases where the first strong character on
...
In this example, the second cue will have a right-to-left base direction, rendering as
   "
...
To change that line to left-to-right base direction, start the line with an U+200E
   LEFT-TO-RIGHT MARK character (it can be escaped as "

&lrm;

").

Where the base direction of some embedded text within a line needs to be different
  from the surrounding text on that line, this can be achieved by using the paired Unicode bidi
  formatting code characters.

In this example, assuming no bidi formatting code characters are used, the cue text is
   rendered as "

I’ve read the book 3 דנליונ
   times!

" (i.e. the "3" is on the wrong side of the book title) because of the effect
...
If a U+2068 FIRST STRONG ISOLATE (FSI) character was placed before the book title and a U+2069
   POP DIRECTIONAL ISOLATE (PDI) character after it, the rendering would be the intended "

I’ve read the book דנליונ 3 times!

".
   (Those characters can be escaped as "
...
regardless of the base direction of the cue text. To make the text alignment of each
  line match the base direction of the line (e.g. left for English, right for Hebrew), use
...
is used. The first
   ...
   right-aligned because the base direction is right-to-left.
...
can be used to left-align or right-align the cue text regardless of
...
change value, then the user agent must empty the
...
represents a subpart of the video viewport and provides a limited
rendering area for
...
An arbitrary string of zero or more characters other than U+0020 SPACE or U+0009 CHARACTER
  TABULATION character. The string must not contain the substring "-->" (U+002D HYPHEN-MINUS, U+002D
  HYPHEN-MINUS, U+003E GREATER-THAN SIGN). Defaults to the empty string.

A width

A number giving the width of the box within which the text of each line of the containing cues
  is to be rendered, to be interpreted as a percentage of the video width. Defaults to 100.

A lines value

A number giving the number of lines of the box within which the text of each line of the
  containing cues is to be rendered. Defaults to 3.

Since a WebVTT region defines a fixed rendering area, a cue that has more lines
...
Two numbers giving the x and y coordinates within the region which is anchored to the video
  ...
  changes. Defaults to (0,100), i.e. the bottom left corner of the region.

A region viewport anchor point

Two numbers giving the x and y coordinates within the video viewport to which the region anchor
  point is anchored. Defaults to (0,100), i.e. the bottom left corner of the video viewport.
...
Indicates that the cues in the region are not to scroll and instead stay fixed at the
...
consists of the following components, in the following order:
...
WebVTT region definition blocks
...
consists of the following components, in the given
order:
...
" (U+0052 LATIN CAPITAL LETTER R, U+0045 LATIN CAPITAL LETTER E,
...
" (U+0053 LATIN CAPITAL LETTER S, U+0054 LATIN CAPITAL LETTER T,
...
Any sequence of zero or more characters other than U+000A LINE FEED (LF) characters and U+000D
 CARRIAGE RETURN (CR) characters, each optionally separated from the next by a
...
Optionally, one or more U+0020 SPACE characters or U+0009 CHARACTER TABULATION (tab) characters
 followed by a
...
, but it must not contain the substring
 "
...
corresponds to one piece of time-aligned text or data in
the

WebVTT file

, for example one subtitle. The
...
is any sequence of one or more characters not containing the
substring "
...
must be unique amongst all the
...
can be used to reference a specific cue, for example
from script or CSS.
...
representing the start time offset of the cue. The time represented
 by this

WebVTT timestamp

must be greater than or equal to the start time offsets of all
...
representing the end time offset of the cue. The time represented by
 this

WebVTT timestamp

must be greater than the start time offset of the cue.

The

WebVTT cue timings

give the start and end offsets of the

WebVTT cue
block

. Different cues can overlap. Cues are always listed ordered by their start time.
...
as a base ten integer in the range
 0 ≤
...
, representing the thousandths of a second
...
is always interpreted relative to the
...
consist of a sequence of zero or more

WebVTT cue settings

in any order, separated from each other by one or more U+0020
...
An optional U+003A COLON (colon) character.
...
When interpreted as a number, a
...
Any sequence of zero or more characters other than U+000A LINE FEED (LF) characters and
...
consists of any sequence of zero or more characters other than
U+000A LINE FEED (LF) characters and U+000D CARRIAGE RETURN (CR) characters, each optionally
separated from the next by a

WebVTT line terminator

. (In other words, any text that does not
have two consecutive
...
cues are only useful for scripted applications (e.g. using the
...
that consists of zero or
more

WebVTT caption or subtitle cue components

, in any order, each optionally separated from
the next by a
...
, representing the text of the cue.

An

HTML character reference

, representing one or two Unicode
 code points, as defined in HTML, in the text of the cue.
...
bar the HTML character reference may have
one or more

cue component class names

attached to it by separating the cue component
...
representing the boldened text, and a
...
". If this is the last occurrence of this
   group of components in the

WebVTT cue ruby span

, then this last end tag string may be
   omitted.
...
If the last end tag string was not omitted: Zero or more U+0020 SPACE characters or U+0009
 CHARACTER TABULATION (tab) characters, each optionally followed by a
...
are more mature in HTML and CSS.
...
" that requires an annotation; the annotation
...
sequence, then the end tag may be omitted for brevity.

A

WebVTT cue language span

consists of the following components, in the order
given:
...
One or more characters other than U+0009 CHARACTER TABULATION (tab) characters, U+000A LINE
...
If the start tag requires an annotation: a U+0020 SPACE character or a U+0009 CHARACTER
...
, representing one or two Unicode
   code points, as defined in HTML, in the text of the annotation.
...
consists of a U+003C LESS-THAN SIGN character (<), followed
by a

WebVTT timestamp

representing the time that the given point in the cue becomes active,
followed by a U+003E GREATER-THAN SIGN character (>). The time represented by the

WebVTT
timestamp

must be greater than the times represented by any previous

WebVTT cue timestamps

in the cue, as well as greater than the cue’s start time
...
that makes use of zero or more of the
following components, each optionally separated from the next by a
...
WebVTT region settings

A

WebVTT cue settings list

can contain a reference to a
...
consists of zero or more of the following components,
in any order, separated from each other by one or more U+0020 SPACE characters, U+0009 CHARACTER
TABULATION (tab) characters, or

WebVTT line terminators

, except that the string must not
contain two consecutive

WebVTT line terminators

. Each component must not be included more
than once per
...
gives configuration options regarding the
...
An arbitrary string of one or more characters other than
...
WebVTT region identifier settings
...
. Without an identifier, it is not possible to associate a
...
gives a name to the region so it can be
...
provides a fixed width as a percentage of the
...
provides a fixed height as a number of lines
...
provides a tuple of two percentages that
...
is given, the anchor defaults to 0%, 100% (i.e. the bottom left
corner).
...
consists of the following components, in the
order given:
...
provides a tuple of two percentages
...
specifies whether cues rendered into the
...
Each of these setting must not be included more than once per
...
One of the following strings as the
...
configures the cue to use vertical text
...
configures the offset of the cue box from the video
viewport’s edge in the direction orthogonal to the

writing
direction

. For horizontal cues, this is the vertical offset from the top of the video viewport,
for vertical cues, it’s the horizontal offset. The offset is for the
...
by default. The offset can be given either as a
...
a position value consisting of: a
...
configures the indent position of the
...
the video viewport. The positioning is for the
...
of the cue box, depending on the cue’s

computed position alignment

, which is overridden by the
...
. For horizontal
cues, this is the width of the

cue box

. It is given as a percentage of
...
configures the alignment of the text within
the cue. The "
...
" keywords are relative to the cue text’s
lines' base direction; for left-to-right English text, "
...
configures a cue to become part of a region by referencing the
region’s identifier unless the cue has a
...
cue setting.
If a cue is part of a region, its cue settings for
...
whose cues all follow the following rules is said to be a
...
with start and end time offsets (
...
In this ninety-second example, the two cues partly overlap, with the first ending before the
 second ends and the second starting before the first ends. This therefore is not a
...
Conformance checkers, when validating

WebVTT files

, may offer to restrict syntax checking
...
background colors for text. Therefore, the WebVTT spec makes available a set of default
...
matching those in the first cell of a row in the table below must set their
...
to the value in the second cell of the
row:
...
If your background is captioning, don’t get confused: The color for the class

lime

is what has traditionally been used in captioning under the name
...
and make sure to take into account the text color, background color and
...
to the value in the second cell
of the row:
...
is what has traditionally been used in
captioning under the name
...
, must decode the byte
stream using the

UTF-8 decode

algorithm, and then must parse the resulting
...
, specifically its conversion and parsing steps, is typically run
asynchronously, with the input byte stream being updated incrementally as the resource is
downloaded; this is called an
...
verifies a file signature before parsing the provided byte stream. If the
...
be the string being parsed, after conversion to Unicode, and with the following
...
, initially pointing at the start of the string. In
 an

incremental WebVTT parser

, when this algorithm (or further algorithms that it uses) moves
 the

position

pointer, the user agent must wait until appropriate further characters from the byte
 stream have been added to

input

before moving the pointer, so that the algorithm never reads past
 the end of the

input

string. Once the byte stream has ended, and all characters have been added
 to
...
pointer may, when so instructed by the algorithms, be moved past
 the end of
...
is less than six characters long, then abort these steps. The file does not start
 with the correct

WebVTT file

signature and was therefore not successfully
 processed.
...
is exactly six characters long but does not exactly equal "
...
is more than six characters long but the first six characters do not exactly
 equal "

WEBVTT

", or the seventh character is not a U+0020 SPACE character, a U+0009
 CHARACTER TABULATION (tab) character, or a U+000A LINE FEED (LF) character, then abort these steps.
 The file does not start with the correct

WebVTT file

signature and was therefore not
...
, then abort these steps. The file was successfully
 processed, but it contains no useful data and so no
...
is a U+000A LINE FEED (LF) character. Advance

position

to the next character in
...
: If the character indicated by

position

is not a U+000A LINE FEED (LF)
 character, then
...
that are U+000A LINE FEED (LF) characters.
...
that are U+000A LINE FEED (LF)
   characters.

End

: The file has ended. Abort these steps. The
...
When the algorithm above says to

collect a WebVTT block

, optionally with a flag

in
header

set, the user agent must run the following steps:
...
be the same variables as those of the same
...
be true. Otherwise, the character
   indicated by
...
contains the three-character substring "
...
is not set and at least one of the following conditions are true:
...
be null. Otherwise, let

buffer

be the empty string and let
...
is the empty string, break out of
...
" (U+0052 LATIN CAPITAL LETTER R, U+0045 LATIN CAPITAL LETTER E, U+0047
        LATIN CAPITAL LETTER G, U+0049 LATIN CAPITAL LETTER I, U+004F LATIN CAPITAL LETTER O, U+004E
        LATIN CAPITAL LETTER N), and the remaining characters in
...
is not the empty string, append a U+000A LINE FEED (LF) character to
...
. If it returned a list of rules, assign the list as
...
is a conceptual construct to represent a

WebVTT region

that is used as a root node for

lists of WebVTT node
objects
...
does not contain a U+003A COLON character (:), or if the first U+003A COLON
   character (:) in

setting

is either the first or last character of
...
be the leading substring of

setting

up to and excluding the first U+003A
...
be the trailing substring of

setting

starting from the character immediately
...
does not contain a U+002C COMMA character (,), then jump to the step
       labeled
...
up to and excluding the first U+002C
       COMMA character (,) in that string.
...
starting from the character
       immediately after the first U+002C COMMA character (,) in that string.
...
: Continue to the next setting, if any.
...
does not match the syntax for a
...
is an error, is less than 0, or is greater than 100, then fail.
...
, initially pointing at the start of the
 string.
...
. If that algorithm fails, then abort these steps and
 return failure. Otherwise, let
...
contains a U+002C COMMA character (,), then let
...
up to and excluding the first U+002C COMMA character (,) in that string
       and let
...
is a U+0025 PERCENT SIGN character (%)
...
, otherwise jump to the step labeled
...
contains any characters other than U+002D HYPHEN-MINUS characters
           (-),

ASCII digits

, and U+002E DOT character (.), then jump to the step labeled
...
other than the first character is a U+002D
           HYPHEN-MINUS character (-), then jump to the step labeled

next setting

.

If there are more than one U+002E DOT characters (.), then jump to the step
           labeled
...
If there is a U+002E DOT character (.) and the character before or the character
           after is not an

ASCII digit

, or if the U+002E DOT character (.) is the first or
           the last character, then jump to the step labeled
...
is an error, then jump to the step labeled
...
is not null, then jump to the step labeled
...
is a U+0025 PERCENT SIGN character (%), then let
...
be false. Otherwise, let it be true.
...
be null (the cue has been explicitly positioned with a
       line offset and thus drops out of the region).
...
’s value remains the special value
...
be
       null (the cue has been explicitly sized and thus drops out of the region).
...
is not exactly two characters in length, or if
...
or if the character at

position

is not a U+003A
 COLON character (:), then return an error and abort these steps. Otherwise, move
...
is not exactly two characters in length, return an error and abort these
 steps.
...
is a U+003A COLON character (:), run these substeps:
...
is not a U+003A COLON character (:)), let
...
is not a U+002E
 FULL STOP character (.), then return an error and abort these steps. Otherwise, move
...
is not exactly three characters in length, return an error and abort these
 steps.
...
is a conceptual construct used to represent components of

cue
text

so that its processing can be described without reference to the underlying syntax.
...
elements in HTML or the DOM.

WebVTT Internal Node Objects

have an ordered list of child
...
of the children. Cycles do not occur; the
parent-child relationships so constructed form a tree structure.

WebVTT Internal Node Objects

also have an ordered list of
...
, and a language, known as their

applicable language

, which is to be interpreted as a BCP 47 language tag.

[BCP47]

User agents will add a language tag as the

applicable language

even if it is not a valid or not even well-formed language tag.

[BCP47]

There are several concrete classes of

WebVTT Internal Node
Objects
...
These are used as root nodes for trees of
...
These represent spans of text (a
...
, and are used
  to annotate parts of the cue with

applicable
  classes

without implying further meaning (such as italics or bold).

WebVTT Italic Objects

These represent spans of italic text (a
...
These represent spans of bold text (a
...
These represent spans of underline text (a
...
WebVTT
  caption or subtitle cue text

.

WebVTT Ruby Objects

These represent spans of ruby (a

WebVTT cue ruby span

) in

WebVTT caption or subtitle
  cue text
...
These represent spans of ruby text (a
...
These represent spans of text associated with a specific voice (a
...
, and are used to annotate parts of the cue where the

applicable language

might be different than the surrounding
  text’s, without implying further meaning (such as italics or bold).

WebVTT Leaf Node Objects

are those that contain data,
such as text, and cannot contain child

WebVTT Node Objects

.

There are two concrete classes of
...
A fragment of text. A

WebVTT Text Object

has a value, which is the text it
  represents.
...
has a value, in seconds and fractions of a
  second, which is the time represented by the timestamp.

The

WebVTT cue text parsing rules

consist of the following algorithm. The input is a
string
...
be a stack of language tags, initially empty.
...
Run the appropriate steps given the type of
...
whose value is the value of the string token
...
How the start tag token

token

is processed depends on its tag name, as follows:
...
Push the value of the token’s annotation string, or the empty string if there is no
      annotation string, onto the
...
Set the new object’s list of

applicable
     classes

to the list of classes in the token, excluding any classes that are the empty
     string.
...
to the top entry on the

language stack

, if the stack is not empty.

Append the newly created node object to
...
If any of the following conditions is true, then let
...
The tag name of the end tag token
...
Otherwise, if the tag name of the end tag token
...
be the parent node of the parent node of
...
If that algorithm does not fail, and if

position

now points at the end of

input

(i.e.
      there are no trailing characters after the timestamp), then create a

WebVTT Timestamp
      Object

whose value is the collected time, then append it to
...
is as follows. It emits a token, which is either a
...
Jump to the entry that matches the value of
...
Otherwise, return a string token whose value is
...
Return a string token whose value is
...
Otherwise, append the data of the character tokens that were returned to

result

.

Then, in any case, set
...
, then jump to the next "end-of-file
      marker" entry below.

End-of-file marker

Return a start tag whose tag name is the empty string, with no classes and no annotation,
      and abort these steps.
...
Return a start tag whose tag name is

result

, with no classes and no annotation, and abort
      these steps.
...
, then return a start tag whose tag
      name is

result

, with the classes given in

classes

but no annotation, and abort these
      steps.
...
with a single U+0020 SPACE character; then, return a start tag whose tag name is
...
as the annotation, and abort these
      steps.
...
Return an end tag whose tag name is
...
Return a timestamp tag whose tag name is
...
When the algorithm above says to attempt to
...
When the HTML specification says to consume a character, in this context, it means to advance
...
. When it says to unconsume a character, it means to move

position

back to the previous character in

input

. "EOF" is equivalent to the end-of-file marker
in this specification. Finally, this context is

not

"as part of an attribute" (when it
...
For the purpose of retrieving a
...
interface, it needs to be parsed to a
...
, user
agents must create a tree of DOM nodes that is isomorphous to the tree of

WebVTT Node Objects

, with the following mapping of
...
, with all optional components
   included, with one leading zero if the

hours

component is less than ten, and with no leading
   zeros otherwise.

HTML elements created as part of the mapping described above must have their
...
, use the appropriate IDL interface as defined
in the HTML specification, and, if the corresponding
...
attribute set to the string obtained by concatenating all those classes, each
...
WebVTT rules for extracting the chapter
title
...
Return the concatenation of the values of each
...
, in a
 pre-order, depth-first traversal, excluding
...
This section describes in some detail how to visually render

WebVTT caption or
subtitle cues

in a user agent. The processing model is quite tightly linked to media elements in
HTML, where CSS is available.

User agents that do not support CSS

are expected to render
plain text only, without styling and positioning features.

User agents that do not support a full
HTML CSS engine

are expected to render an equivalent visual representation to what a user agent
...
element), or of another playback
mechanism, by applying the steps below. All the
...
, or other playback mechanism, are rendered together, to avoid
overlapping subtitles from multiple tracks. A fallback language

language

may be set when calling
this algorithm.
...
The output of the steps below is a set of CSS boxes that covers the rendering area of the

media element

or other playback mechanism, which user agents are expected to render in a
...
element, or is another playback
...
or other playback mechanism.

Let

output

be an empty list of absolutely positioned CSS block boxes.
...
one or more completely transparent positioned CSS block
 boxes that cover the same region as the user interface.

If the last time these rules were run, the user agent was not
...
Apply the following steps for each

regionNode

:

Prepare some variables for the application of CSS properties to
...
Apply the terms of the CSS specifications to

regionNode

within the following constraints,
    thus obtaining a CSS box

box

positioned relative to an initial containing block:

No style sheets are associated with

regionNode

. (The regionNodes are subsequently
...
have their values set as defined in the next section. (That
...
is not null, add those boxes to that region’s
...
that has not yet had corresponding CSS boxes added
  to
...
is null, run the following substeps:
...
have the CSS boxes in

boxes

.

Add the CSS boxes in
...
and divided by 100 (i.e. interpret it as a percentage
     of the region width).
...
positioned relative to an initial containing
     block.

If there are no line boxes in

boxes

, skip the remainder of these substeps for
...
together have a height less than the height of the
...
be the absolute difference between the two height values. Increase
...
dragging them to another location on the
...
When the processing algorithm above requires that the user agent

apply WebVTT cue
settings

to obtain CSS boxes from a
...
be "horizontal-tb". Otherwise, if the
...
as per the appropriate rules from the following
  list:
...
from 100 and then multiplying the result by two.
...
.
 (These are CSS values used by the next section to set CSS properties for the rendering;
...
as per the appropriate rules from
  the following list:
...
Determine the value of whichever of
...
. (These are
 CSS values used by the next section to set CSS properties for the rendering;
...
according to the appropriate steps from the following list:
...
" must be followed only when the
...
be the height of the first line box in
...
be the width of the first line box in
...
is zero, then jump to the step labeled
...
to an integer by adding 0.5 and then flooring it.
...
is less than zero then increase
...
right by the distance given by

position

.

Remember the position of all the boxes in
...
be a box that covers all of the
...
: If none of the boxes in

boxes

would overlap any of the boxes in

output

, and all of the boxes in
...
is negative and the top of the first line box in

boxes

is now above the top of the
...
is positive and the bottom of
      the first line box in

boxes

is now below the bottom of the
...
is negative and the left edge of the first line box in

boxes

is now to the left of the left edge of the
...
is positive and
      the right edge of the first line box in

boxes

is now to the right of the right edge of the
...
is negative, then this will actually result in an upwards movement of the
...
is negative, then this will actually result in a leftwards movement of the
      boxes in absolute terms.)

Jump back to the step labeled
...
is true, then remove all the boxes in
...
Otherwise, move all the boxes in
...
be the bounding box of the boxes in
...
’s rendering area, then jump to the step labeled
...
If there is a position to which the boxes in

boxes

can be moved while maintaining the
     relative positions of the boxes in

boxes

to each other such that none of the boxes in
...
’s rendering area, then move the boxes in

boxes

to the closest such position to their
     current position, and then jump to the step labeled
...
, then apply the terms of the CSS specifications to
...
For the purpose of selectors in STYLE blocks of a WebVTT file, the style sheet must apply to a
  ...
  attributes, no classes, no IDs, and unknown primary language, that acts like the
...
that were sourced from the given WebVTT file. The selectors
...
This element exists only to be the
...
of the declarations in STYLE
  ...
  after any style sheets that apply to the associated
...
declaration would win, because it is last in the
...
Supporting external resources with
...
are equivalent to elements with the same
 contents.
...
. (The nodes are subsequently restyled using style
...
must be wrapped in an anonymous box whose
...
must be wrapped in anonymous boxes whose
...
The video viewport (and initial containing block) is
...
be the boxes generated as descendants of the initial containing block, along with
their positions.
...
at the CSS user agent cascade
layer as defined in this section.
...
with the following CSS settings:
...
are the values with those
names determined by the
...
shorthand property on the (root)
...
is initialized with the following CSS settings:
...
shorthand property must be set to
...
are the values with those names determined by
the
...
are further initialized with these CSS
settings:
...
property must be set as described for the root
...
must inherit their values from the
...
is being rendered, if any. If there is no
...
is being rendered for another media playback mechanism), then inherited
...
WebVTT region objects

must take their initial values.

If there are style sheets that apply to the
...
apply to WebVTT. This section does not apply to
...
pseudo-element represents a cue.
...
pseudo-element represents a cue or element inside a cue that match the
given selector.
...
pseudo-element represents a region.
...
pseudo-element represents a region or element inside a region
...
matching the given language range.
...
When a user agent is rendering one or more
...
used in the rendering can be matched by
certain pseudo-selectors as defined below. These selectors can begin or stop matching individual
...
is being
rendered, even in between applications of the

rules for updating the display of WebVTT text
tracks

(which are only run when the set of active cues changes). User agents that support the
pseudo-element described below must dynamically update renderings accordingly. When either

white-space

or one of the properties corresponding to the
...
. The pseudo-elements defined in the following sections
affect the styling of parts of

WebVTT cues

that are being rendered for the
...
A CSS user agent that implements the
...
pseudo-element (with no argument) matches any
...
, with the exception that the properties
corresponding to the

background

shorthand must be applied to the
...
pseudo-element with no argument; other properties
...
the properties corresponding to the
...
pseudo-element with an argument must have an argument that
...
that also matches the given CSS selector, with the nodes
...
against which the selectors are matched is the tree of
...
For the purposes of element type selectors, the names of

WebVTT Internal Node Objects

are as given by the following table, where objects having
...
For the purposes of element type and universal selectors,

WebVTT Internal Node Objects

are considered as being in the namespace expressed as the
 empty string.

For the purposes of attribute selector matching,

WebVTT
 Internal Node Objects

have no attributes, except for

WebVTT Voice
 Objects

, which have a single attribute named "
...
, which
 have a single attribute named "

lang

" whose value is the object’s
...
For the purposes of class selector matching,

WebVTT
 Internal Node Objects

have the classes described as the

WebVTT Node Object’s applicable
 classes
...
have the language described as the
...
For the purposes of ID selector matching,

lists of
 WebVTT Node Objects

have the ID given by the cue’s
...
pseudo-element with an argument:
...
In addition, the following properties apply to the

::cue()

pseudo-element with an argument
when the selector does not contain the
...
As a special exception, the properties corresponding to the

background

shorthand, when they
would have been applied to the

list of WebVTT Node Objects

, must instead be applied to the
...
pseudo-classes sometimes match
...
if, in a
pre-order, depth-first traversal of the
...
pseudo-element (with no argument) matches any list of
...
No other selector matching is defined for
...
The same properties that apply to
...
When a user agent is rendering one or more text track regions according to the

rules for
updating the display of WebVTT text tracks
...
used in the rendering can be matched by the above pseudo-element. User agents that
...
in
the region must be emptied and the
...
object to which this cue belongs, if any, or null otherwise.
...
. In the case of the value being
...
object
represents, if any; or null otherwise. On setting, the
...
attribute, on getting, must return the string from
the second cell of the row in the table below whose first cell is the
...
must be set to the value given in the first
cell of the row in the table above whose second cell is a
...
object represents
is true; or false otherwise. On setting, the
...
object represents. The special value

auto

must be represented as the string "
...
must be set to the new value; if the new value is the string
"

auto

", then it must be interpreted as the special value
...
attributes
in any order, the API does not reject setting
...
must be set to the value given in the first cell
of the row in the table above whose second cell is a
...
must be set to the new value; if the new
value is the string "
...
attribute, on getting, must return the string
from the second cell of the row in the table below whose first cell is the
...
attribute, on getting, must return the string from the
...
attribute, on getting, must return the raw
...
A fallback language is not provided for
...
cannot expose language information.
...
Returns the WebVTT region width as a percentage of the video width. Can be set. Throws an
...
Returns the text track region height as a number of lines. Can be set. Throws an
...
Returns the WebVTT region anchor X offset as a percentage of the region width. Can be set.
  Throws an
...
Returns the WebVTT region anchor Y offset as a percentage of the region height. Can be set.
  Throws an
...
Returns the WebVTT region viewport anchor X offset as a percentage of the video width. Can be
  set. Throws an
...
Returns the WebVTT region viewport anchor Y offset as a percentage of the video height. Can be
  set. Throws an
...
constructor, when invoked, must run the
following steps:
...
text track region regionAnchorX
...
text track region regionAnchorY
...
text track region viewportAnchorX
...
text track region viewportAnchorY
...
object represents. On setting,
if the new value is negative or greater than 100, then an

IndexSizeError

exception must be
thrown. Otherwise, the
...
exception must be thrown. Otherwise, the

WebVTT region anchor

X distance must be set to the
new value.
...
must be set to the value given on the first cell of
the row in the table above whose second cell is a
...
Text track files themselves pose no immediate risk unless sensitive information is included
...
WebVTT files all begin with one of the following byte sequences (where "EOF" means the end of
    the file):
...
(An optional UTF-8 BOM, the ASCII string "
...
Silvia Pfeiffer <silviapfeiffer1@gmail.com>, Simon Pieters <simonp@opera.com>, Philip
...
cannot fetch any external resources
...
for pseudo-elements other than
...
the origin server that the user has a need for captions or subtitles, and also the

language preference

of the user for captions or
