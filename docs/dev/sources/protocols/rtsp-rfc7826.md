...
> Fetched: 2026-01-30T23:52:26.926851+00:00
> Content-Hash: 08dd5cc299e5509a
> Type: html
...
. Message Body Format Negotiation ...........................
...
. Secured Media Session for On-Demand Content ..............
...
.  There are many reasons why RTSP 2.0 can't be
...
, the Session Description Protocol
   (SDP) usage with RTSP is defined in
...
, are three normative specification
...
Section 13.2
   ...
   resources and aggregates under common control (see

Section 4.2

).

   This specification describes in
...
) to the server.  In the Transport header
   (
...
Section 18.49
   ...
   selected identifier in the Pipelined-Requests header (
...
) informs the client about range formats that the
   server supports for these media resources.  The Media-Range header
   (

Section 18.30

) informs the client about the time range of the media
   currently available.
...
After having established an RTSP session, the client can start
   ...
   playback", using the PLAY method (

Section 13.4

) and "suspend (pause)
   playback" by using the PAUSE method (
...
).  The Range header also allows the client to
...
This document specifies how media is delivered with RTP [
...
], or the RTSP connection.  Additional
...
) is used for fast-forward or slow-motion control
...
) affects how much of the playback timeline is
...
discusses the methods for showing liveness in more
...
) and the Supported header (

Section 18.51

).  It
   can also try and possibly fail using new methods or require that
   particular features be supported using the Require (
...
for more information.
   ...
   IRI:  An Internationalized Resource Identifier is similar to a URI
      ...
      (Unicode/ISO 10646), rather than the US-ASCII only.  See [
...
and transmitted over a transport between RTSP agents.
...
.
   ...
   Presentation description:  A presentation description contains
      ...
      about the content.  Other IETF protocols, such as SDP ([
...
Appendix B
   ...
   URI:  A Universal Resource Identifier; see [
...
RTSP 2.0 defines and registers or updates three URI schemes "rtsp",
   ...
   o  Support for an IPv6 literal in the host part and future IP
      literals through a mechanism defined in [
...
Neither should have any significant impact on interoperability.  If
   ...
   This specification also defines the format of RTSP IRIs [
...
].  This allows a URI that matches the
...
RFC3987
   ...
   The "scheme" and "host" parts of all URIs [
...
] are case insensitive.  All other parts of RTSP URIs and
   IRIs are case sensitive, and they MUST NOT be case mapped.

   The fragment identifier is used as defined in Sections
...
], i.e., the fragment is to be stripped from the IRI by the
...
Please note that relative URIs with queries are difficult to handle
   due to the relative URI handling rules of
...
].  URIs may refer to a stream or an aggregate of streams;
...
Session identifiers are strings of a length between 8-128 characters.
   ...
   cryptographically random (see [

RFC4086

]).  It is RECOMMENDED that a
   session identifier contain 128 bits of entropy, i.e., approximately
   22 characters from a high-quality generator (see
...
RTSP currently supports three different media-time formats defined
   ...
   These time formats can be used with the Range header (
...
).  The unqualified format identifier is used on its
   own in Accept-Ranges header (

Section 18.5

) to declare supported time
...
A timestamp may use a format derived from a Society of Motion Picture
   ...
   are expressed as SMPTE time codes [
...
The syntax is based on ISO 8601 [
...
] ) using colons (":") as separators between hours,
...
].  The date is a complete representation of the
   calendar date in basic format (YYYYMMDD) without separators (per
   Section 5.2.1.1 of [

ISO.8601.2000

]).  The time of day is provided in
   the complete representation basic format (hhmmss) as specified in
   Section 5.3.1.1 of [

ISO.8601.2000

], allowing decimal fractions of
   seconds following

Section 5.3.1.3

requiring "." (full stop) as
...
] are historic.  We continue
      to use the format specified in RTSP 1.0.  The motivations raised
      in

RFC 3339

apply to why a selection from ISO 8601 was made;
...
, "IANA Considerations".)
   ...
   The usage of feature tags is further described in
...
Appendix D.1.9
   ...
   MTag is similar to ETag in HTTP/1.1 (see

Section 3.11 of [RFC2068]
...
for information on the If-Match
...
RFC3629
   ...
   is also the encoding used for text fields in RTCP [
...
) messages use a format based on
   the generic message format of
...
RFC5322
   ...
   message body.  The ABNF [

RFC5234

] below is for illustration only; the
...
RTSP header fields (see

Section 18

) include general-header, request-
...
) and include them in the appropriate
   Require (
...
)) does not allow sending a message body.  In case a
...
|
   ...
   Detailed header definitions are provided in
...
] status codes and then added RTSP-specific status codes
...
).  To enable the responder of a request to
   determine which media type it should use, the requester may include
   the Accept header (

Section 18.1

) in a request to identify supported
...
attempted to specify an optional mechanism for transmitting
...
A TCP transport can be used for both persistent connections (for
   ...
   support RTSP over TCP.  The scheme of the RTSP URI (
...
In light of the above, it is RECOMMENDED that clients use persistent
   ...
   connections MAY "pipeline" its requests (see
...
The client MAY close a connection at any point when no outstanding
   ...
   connection have been timed out (
...
SET_PARAMETER:  When using SET_PARAMETER for keep-alives, a body
   ...
   The timeout parameter of the Session header (
...
Section 18.49
   ...
   closing the session due to lack of activity (see
...
RFC2460
...
] support was not present in RTSP 1.0.  RTSP
   ...
   address fields in the Transport header (
...
) to let servers and proxies notify requesting proxies
...
Pipelining is a general method to improve performance of request/
   ...
   indicated by the Pipelined-Request header (see
...
The semantics of the RTSP OPTIONS method is similar to that of the
   HTTP OPTIONS method described in
...
.  An OPTIONS request intended for
...
) to indicate which time formats are acceptable to
...
).  The combination of the parameters of the Media-
   Properties header indicates the nature of the content present in the
   session (see also

Section 4.7

).  For example, a live stream with time
...
) if the media is Time-Progressing.
...
for aggregated sessions and for the
   particular URIs see

Appendix D.1.1

).  The aggregate control URI is to
...
The PLAY method tells the server to start sending data via the
   ...
   accordance with the Seek-Style header (

Section 18.47

).  It MUST
   deliver stream data until the end of the range if given, until a new
   PLAY request is received, until a PAUSE request (
...
.  The pause point for media that is
   ...
   error code and include the Media-Range header (
...
Section 18.30
   ...
   include the Media-Range header (
...
Section 18.46
...
) is included with a negative value
   (playing backwards).
   ...
   by including the Seek-Style header (
...
, and used in the following example.
...
For aggregated sessions where the initial SETUP request (creating a
   ...
   request MAY be pipelined (

Section 12

) after those additional SETUP
...
as long as the media has not been changed.
...
Section 13.5.2
   ...
   the current Media-Range header (see
...
Live media is indicated by the content of the Media-Properties header
   in the SETUP response when (see also
...
Section 18.30
...
) for this type of media.  For live media with
   ...
   recording.  The considerations in

Section 13.5.3

apply if a client
   requests delivery with scale (
...
Section 18.30
   ...
   the recording.  The considerations in

Section 13.5.3

apply if a
   client requests delivery using a scale (
...
Section 13.2
   ...
   The Notify-Reason header (see

Section 18.32

) specifies the reason why
   the server sends the PLAY_NOTIFY request.  This is extensible and new
   reasons can be added in the future (see

Section 22.8

).  In case the
   client does not understand the reason for the notification, it MUST
   respond with a 465 (Notification Reason Unknown) (
...
A PLAY_NOTIFY request with the Notify-Reason header set to end-of-
   ...
   request.  The Request-Status header (

Section 18.42

) MUST be included
   to indicate which request the notification is for and its completion
   status.  The message response status codes (
...
) or the available media range that
   can be played as indicated by the Media-Range header (
...
) value other than 1.0 (normal playback rate).  For
...
) is used to specify which format the message
...
Section 18.19
   ...
   considerations regarding message body negotiation, see
...
.
   ...
   regarding message body negotiation, see
...
) to inform the client of the reason for the request.
...
, the server MAY terminate the session or transport
...
In order to fulfill certain requirements on the network side, e.g.,
   ...
   unsigned integer in network octet order (Appendix B of [
...
Section 18.54
   ...
   interleaved parameter of the Transport header (see
...
RTSP Proxies are RTSP agents that are located in between a client and
   ...
   Caching Proxy:  This type of proxy is used to reduce the workload on
         ...
         has been cached and has not become stale.  See
...
.  However, that trust model may not be suitable
...
defines the handling of how requests and responses are
   rewritten.
...
When a cache has a stale entry that it would like to use as a
   ...
   for the requested resource, and, if they match (see
...
Section 16.1.3
   ...
   and only if no validators match.
      ...
      prohibited by a cache directive (see
...
The MTag response-header field-value, a message body tag, provides
   ...
   Message body tags are described in
...
.  Rules for When to Use Message Body Tags and Last-Modified Dates
...
Where applicable, HTTP status codes (see

Section 6 of [RFC7231]

) are
   reused.  See Table 4 in

Section 8.1

for a listing of which status
...
The requesting agent SHOULD continue with its request.  This interim
   ...
   completed, continue to wait for a final response (see
...
18.26
...
) and the requested resource has not been
   ...
   unchanged.  The MTag and If-Match header (
...
].  The usage of the error code is
   defined in [

RFC7235

] and any applicable HTTP authentication scheme,
   such as Digest [

RFC7616

].  The response is to include a WWW-
   Authenticate header (
...
Section 17.4.2
   ...
   proxy.  The usage of this error code is defined in [

RFC7235

] and any
   applicable HTTP authentication scheme, such as Digest [
...
The precondition given in one or more of the 'if-' request-header
   ...
   sections for the 'if-' headers: If-Match
...
The Accept request-header field can be used to specify certain
   presentation description and parameter media types [
...
] in Base64, according to

Section 4 of
   [RFC4648]

and where the padding bits are set to zero.  All RTSP
...
The Accept-Encoding request-header field is similar to Accept, but it
   restricts the content-codings (see
...
] in the response message.  The definition of
   the header is in [

RFC7615

], and any applicable HTTP authentication
   schemes appear in other RFCs, such as Digest [
...
An RTSP client that wishes to authenticate itself with a server using
   the authentication mechanism from HTTP [
...
], and any applicable HTTP authentication schemes appear in
   other RFCs, such as Digest [
...
) receives a request containing an
...
Language tags are mentioned in
...
], to synchronize its clock with
   a reliable external standard.

   The RTSP-date, a full date as specified by
...
.  This format is more flexible than the date
      format in

RFC 1123

used by RTSP 1.0.  Thus, implementations should
      use single spaces as separators, as recommended by
...
for rules on how to determine if two message body
   tags match.
...
Retention:
      ...
      Time-Limited:  Content will be retained at least until the
         ...
         absolute time format specified in
...
) when setting up the media.  The server MAY include
...
The MTag response-header MAY be included in DESCRIBE, GET_PARAMETER,
   or SETUP responses.  The message body tags (
...
Section 4.6
   ...
   the session description as specified in
...
The Notify-Reason response-header is solely used in the PLAY_NOTIFY
   ...
   asynchronous PLAY_NOTIFY request (see
...
The Proxy-Authenticate response-header field MUST be included as part
   ...
   definition of the header is in [

RFC7235

], and any applicable HTTP
   authentication schemes appear in other RFCs, such as Digest [
...
], and any applicable HTTP authentication schemes appear
...
for more details on the mechanics of this message
   and a usage example.  See discussion in the proxies section
   (

Section 15.1

) about when to consider that a feature requires proxy
   support.
...
The Proxy-Supported general-header field enumerates all the
   ...
   applicable to proxies, as described in
...
) MAY be used to indicate methods allowed for a
   particular URI.
...
) requests and responses.  It MAY be
...
) range units.  While octet ranges (Byte Ranges) (see

Section 2.1 of [RFC7233]

) and other extended units MAY be used, their
...
) indicates a negative scale value.  For example, this
...
This request-header is used to indicate the end result for requests
   that take time to complete, such as PLAY (
...
) with the end-of-stream reason to report
...
can reduce the issue by using a monotonically increasing counter
   ...
   numerical status code (according to
...
) is much more effective in this purpose.  In case any
...
) with the exception of media-modifying proxies.
...
).  See discussion in the proxies section
   (
...
.  The RTP-Info header and the Range header MAY be
...
The Scale general-header indicates the requested or used view rate
   ...
   require certain considerations to work consistently; see
...
).  The client SHOULD only indicate request
...
, that are understood by the client or server.  These
...
The Transport general-header indicates which transport protocol is to
   ...
   code 461 (Unsupported Transport) (
...
) with this method that need to be addressed because
...
; in this specification, it is unspecified
...
.  The argument provides the channel number to be
         used in the $ block (see

Section 14

) and MUST be present.  This
   ...
   MIKEY:  This parameter is used in conjunction with transport
         specifications that can utilize MIKEY [
...
] before being included in the value
         part of the parameter, where the encoding adheres to the
         definition in

Section 4 of RFC 4648
...
RFC5761
...
] on a single underlying transport stream/flow.  The
   ...
   setup:  Clients use the setup parameter on the Transport line in a
         ...
         connection.  This parameter is adapted from [
...
The Unsupported response-header lists the features not supported by
   ...
   specified via the Proxy-Require field (
...
The WWW-Authenticate header is specified in [

RFC7235

]; its usage
   depends on the used authentication schemes, such as Digest [
...
].  The WWW-Authenticate response-header field MUST
...
.  User agents are advised to take
...
RTSP and HTTP share common authentication schemes; thus, they follow
   the same framework as specified in [
...
RFC7235
   ...
   by importing the definitions from [

RFC7235

].  Servers SHOULD
   implement both the Basic [
...
RFC7616
   ...
   considerations specified below in
...
.  Any RTSP message containing an Authorization
...
), and the Proxy-Authentication-Info (
...
) headers are end-to-end and MUST NOT be modified
   by proxies.
...
].  These modifications are based on the procedures defined
   for SIP 2.0 [
...
.
   ...
   Info and Proxy-Authentication-Info, that are defined as in [
...
RFC7616
...
], with "HTTP/1.1" replaced by "RTSP/2.0" in addition to the
   ...
   1.  Use the ABNF specified in the referenced documents, with the
       ...
       RTSP, i.e. the ABNF element: Request-URI (see
...
] usage as specified for HTTP; see [
...
) is used, where the client can request the proxy or
...
Proxy:  For the "proxy" method, the proxy (or proxies) MUST use its
   ...
   User: For the "user" method, the proxy (or proxies) MUST send
         ...
         proxy should accept the certificate.  See
...
, with the next hop's identity and
      certificate.
...
qdtext          = %x20-21 / %x23-5B / %x5D-7E / quoted-pair
   ...
   UTF8-1           = <As defined in

RFC 3629

>
   UTF8-2           = <As defined in

RFC 3629

>
   UTF8-3           = <As defined in

RFC 3629

>
   UTF8-4           = <As defined in

RFC 3629

>
   UTF8-tail        = <As defined in
...
; not including ";"
   ...
   iquery         =  < As defined in

RFC 3987

>
   iunreserved    =  < As defined in

RFC 3987

>
   pct-encoded    =  < As defined in
...
[ encoding *(COMMA encoding) ]
   ...
   auth-param-list   =  <As the Authentication-Info element in
...
If-Modified-Since  =  "If-Modified-Since" HCOLON RTSP-date
   ...
   challenge-list     = <As defined by the WWW-Authenticate in
...
Unsupported     = "Unsupported" HCOLON feature-tag-list
   ...
   IPv4address     = < As defined in

RFC 3986

>
   IPv6address     = < As defined in
...
for the definition of the extensions in text.
...
There are several chapters and an appendix in this document that
   ...
   should take special notice of the Security Framework (
...
This section focuses on issues related to the signaling protocol.
   ...
   and HTTP servers, the security considerations outlined in [
...
RFC7235
   ...
   Abuse of Server Log Information:  A server is in the position to save
         ...
         guidelines followed for its analysis.  See
...
resource content and the signaling protocol mitigates the risk
         ...
         for RTSP signaling using proxies (
...
], future HTTP specifications may provide
...
] authentication.  In environments requiring tighter
...
), one really needs to be aware of
...
Resource Exhaustion:  As RTSP is a stateful protocol and establishes
         ...
         regarding the handling of connections in
...
): namely client authentication using
...
The set of general threats from or by the media-stream delivery
   ...
   Concentrated Denial-of-Service Attack:  The protocol offers the
      ...
      stream is the hammer in that DoS attack.  See
...
) a protocol-specific analysis of security
...
] requests a cookie (consisting of random material) that the
...
RTP is a commonly used media-transport protocol and has been the most
   ...
   properties and the RTP security consideration (
...
] RTCP extensions enables both the lock down of the
      bitrate to low values and disruption of video quality by
      requesting intra-frames.

   Taking into account the above general discussion in
...
RTP.  Therefore, this specification has the following requirements on
   ...
   RTSP agents supporting RTP MUST implement Secure RTP (SRTP) [

RFC3711

]
   and, thus, SAVP.  In addition, SAVPF [
...
RFC5124
   ...
   specified as mandatory to implement in
...
, i.e., AES-CM and
   ...
   necessary functions for MIKEY-RSA-R mode [
...
This section describes a number of registries for RTSP 2.0 that have
   ...
   registers.  For more information on extending RTSP, see
...
.
   ...
   registration policies described in [
...
When a client and server try to determine what part and functionality
   ...
   The usage of feature tags is explained in
...
.  Extending the protocol with new
...
A new method is registered through a Standards Action [
...
A status code is the three-digit number used to convey information in
   RTSP response messages; see

Section 8

.  The number space is limited,
...
].  New RTSP functionality requiring Status Codes should
...
(this document) registers the numbered status code defined
   in the ABNF entry "Status-Code", except "extension-code" (that
   defines the syntax allowed for future extensions) in
...
].  A specification is recommended to be provided, preferably
...
All headers specified in

Section 18 in RFC 7826
...
TS-26234
   ...
   o  x-wap-profile-warning defined in [
...
TS-26234
   ...
   o  x-initpredecbufperiod defined in [
...
TS-26234
   ...
   o  x-initpostdecbufperiod defined in [
...
TS-26234
   ...
   o  3gpp-videopostdecbufsize defined in [
...
TS-26234
   ...
   o  3GPP-Adaptation defined in [

TS-26234

].  This general-header is
...
TS-26234
   ...
   o  3GPP-QoE-Feedback defined in [

TS-26234

].  This request-header is
...
This registry is for policies for an RTSP proxy's handling and
   ...
   connection on behalf of a client.  In

Section 19.3.1

, three policies
   for how to handle certificates are specified.  Further policies may
   be defined; registration is made through Standards Action [
...
The Accept-Credentials header (see
...
].  The registration needs to fulfill the following
   requirement:
...
].  The expert reviewer verifies that a
...
This specification registers the ten values listed in
...
].  The expert reviewer verifies that the
...
) allows for different range formats.
   These range formats also need an identifier to be used in the Accept-
   Ranges header (

Section 18.5

).  New range formats may be registered,
...
A registration follows the Specification Required policy [
...
This registry contains reasons for session termination that can be
   included in a Terminate-Reason header (

Section 18.52

).  Registrations
   follow the Expert Review policy [
...
) in addition to a reason.
   Registrations are made under the Specification Required policy
   [

RFC5226

].  The expert reviewer verifies that the registration
...
This specification registers the following values:
   ...
   RTP/AVP:  Use of the RTP [

RFC3550

] protocol for media transport in
...
RFC 7826, Appendix C.1

.

   RTP/AVP/UDP:  the same as RTP/AVP.

   RTP/AVPF:  Use of the RTP [
...
] over UDP.  The usage is explained in
...
] over TCP.  The usage is explained in
...
The Transport Mode is a Transport header (
...
].  The registration request needs to meet the
   following requirements:
...
Transport Parameters are different parameters used in a Transport
   header's transport specification (
...
RFC5226
   ...
   o  A value definition, if the parameter takes a value, that follows
      the ABNF definition of "trn-par-value"
...
RFC2326
   ...
   created by this memo.  Registrations follow [
...
RFC 7826
   ...
   Interoperability considerations:  The extensions in the URI syntax
         ...
         issues.  The changes are:
            ...
            literals through a mechanism as defined in
...
also apply to this scheme.  They need to
...
RFC 7826
   ...
   Interoperability considerations:  The "rtsps" scheme was never
         ...
         2.0 can create interoperability issues.  The changes are:
            ...
            Support for IPv6 literals in the host part and future IP
            literals through a mechanism as defined by
...
This specification defines three SDP [
...
], but was not formally specified.
...
, DOI 10.17487/RFC2617, June 1999,
              <
...
]  Ignjatic, D., Dondeti, L., Audet, F., and P. Lin, "MIKEY-
...
http://www.rfc-editor.org/info/rfc5124
...
, DOI 10.17487/RFC7617, September 2015,
              <
...
draft-ietf-avtcore-rtp-circuit-breakers-13
...
] Society of Motion Picture and Television Engineers, "ST
...
] 3rd Generation Partnership Project (3GPP), "Transparent
...
, DOI 10.17487/RFC2068, January 1997,
              <
...
, DOI 10.17487/RFC4567, July
              2006, <

http://www.rfc-editor.org/info/rfc4567
...
http://www.rfc-editor.org/info/rfc5104
...
, DOI 10.17487/RFC5583, July 2009,
              <
...
].  It also defines any necessary media-transport
...
RFC3551
...
] when using RTP for media transport over
   ...
   (interleaved) binary data as defined in
...
This part describes the sending of RTP [
...
] according to the profile "RTP Profile
...
, SHOULD NOT be performed when RTSP
...
RFC4585
...
] MAY be used as RTP profiles in sessions using RTP.
   ...
   reporting interval and feedback messages to be used [
...
] is an RTP profile (SAVP) that MAY be used in RTSP sessions
...
.  MIKEY Key Establishment

This method for using MIKEY [

RFC3830

] to establish the SRTP
...
] the client can be the initiator and
...
] and becomes the value of the MIKEY parameter that is
...
].  This allows servers and client to reduce
...
The use of embedded (interleaved) binary data transported on the RTSP
   connection is possible as specified in
...
] according to "Framing Real-time Transport
   Protocol (RTP) and RTP Control Protocol (RTCP) Packets over
   Connection-Oriented Transport" [

RFC4571

] is described.  This section
   adapts the guidelines for using RTP over TCP within SIP/SDP [
...
RFC4145
   ...
   and RTCP on a single transport flow (see
...
.  The server sets only one address specification in the
...
Section 17.4.28
   ...
   using the protocol defined in [

RFC4571

], not by the framing defined
   for interleaving RTP over the RTSP connection defined in
...
.
   ...
   the "connection" parameter (defined in
...
].  Two cases occur, the first is when a new PLAY
...
] states that: "the RTP timestamp for each unit
...
], this matter was not clearly defined and
...
), RTP timestamps should correspond to
   the rendering timing.  For example, when playing video recorded at 30
   frames per second at a scale of two and speed (
...
The client can maintain a correct display of NPT by noting the RTP
   ...
   The sequence parameter of the RTP-Info (
...
BYE message until it has finished using a SSRC.  A server SHOULD keep
   ...
   its SSRC in accordance with the RTP specification [
...
]; this differs from SDP Offer/Answer [
...
RFC5888
   ...
   provides such functionality to some degree.
...
The "a=control" attribute is used to convey the control URI.  This
   ...
   ABNF for the attribute is defined in
...
Example:
   ...
   following the rules and conventions set out in
...
was very unclear on the processing of relative URIs
...
processing to determine the
   ...
   level URI that resolves in accordance with
...
The "m=" field is used to enumerate the streams.  It is expected that
   ...
   currently defined combinations are discussed in
...
The payload type or types are specified in the "m=" line.  In case
   the payload type is a static payload type from
...
], a media type registered with IANA
   according to [

RFC4855

], or an experimental encoding as specified in
   SDP [

RFC4566

]).  Codec-specific parameters are not specified in this
...
Servers MUST take care to provide RTSP Range (see
...
Section 18.40
   ...
   and their values are as defined in
...
In SDP used with RTSP, the "c=" field contains the destination
   ...
   written as "::"), i.e., the unspecified address according to
...
The optional "a=mtag" attribute identifies a version of the session
   ...
   this identifier in the If-Match field (see
...
In this scenario, the server has multiple streams that can be
   ...
   resolved to absolute URIs according to
...
For some types of media, it is desirable to express a relationship
   ...
   synchronization or Scalable Video Codec (SVC) [
...
) or if aggregate control is not available
   (see
...
The above high-level description can be broken down into a number of
   ...
   Presentation Description:  Provide initialization information about
         ...
         descriptions.  Two good examples are the use of HTTP [
...
RFC4566
   ...
   Synchronization:  For media-transport protocols like RTP [
...
), the difference is the nature of the content itself.
...
RFC2974
...
] and SDP are intended to handle.  However, in use cases
   ...
   PLAY_NOTIFY method (see

Section 13.5

) with a suitable extension could
...
but never defined, even if used in examples that
...
defined the "rtspu" URI scheme and provided some
...
The following points should be considered for an interoperable
   ...
   o  The RTT can be estimated as in TCP (
...
.  It should be noted that the CSeq header
...
This section contains notes on issues about backwards compatibility
   with clients or servers being implemented according to
...
].  Note that there exists no requirement to implement RTSP
...
A server implementing RTSP 2.0 MUST include an RTSP-Version of
   ...
   respond with an RTSP 1.0 response if it chooses to support

RFC 2326

.
   If the server chooses not to support
...
Some server implementations of
...
] and RTSP 2.0 for an informational purpose.  For
...
The following protocol elements were removed in RTSP 2.0 compared to
   ...
   o  incorporated much of related HTTP text ([
...
-7235.  Most of the text has been copied and
      then altered to fit RTSP into this specification.  The Public and
      the Content-Base headers have also been imported from
...
*  Removed the possibility of using Range header with Scale header
      ...
      *  The Supported header was borrowed from SIP [
...
] and have been gathered in a separate
         section (
...
are Henning Schulzrinne, Anup Rao, and Robert Lanphier.
...
draft-ietf-mmusic-rfc2326bis-39

draft-ietf-mmusic-rfc2326bis-38

draft-ietf-mmusic-rfc2326bis-37

draft-ietf-mmusic-rfc2326bis-36

draft-ietf-mmusic-rfc2326bis-35

draft-ietf-mmusic-rfc2326bis-34

draft-ietf-mmusic-rfc2326bis-33

draft-ietf-mmusic-rfc2326bis-32

draft-ietf-mmusic-rfc2326bis-31
...
draft-ietf-mmusic-rfc2326bis-26

draft-ietf-mmusic-rfc2326bis-25

draft-ietf-mmusic-rfc2326bis-24

draft-ietf-mmusic-rfc2326bis-23
...
draft-ietf-mmusic-rfc2326bis-17

draft-ietf-mmusic-rfc2326bis-16

draft-ietf-mmusic-rfc2326bis-15

draft-ietf-mmusic-rfc2326bis-14

draft-ietf-mmusic-rfc2326bis-13
...
draft-ietf-mmusic-rfc2326bis-09

draft-ietf-mmusic-rfc2326bis-08

draft-ietf-mmusic-rfc2326bis-07

draft-ietf-mmusic-rfc2326bis-06

draft-ietf-mmusic-rfc2326bis-05

draft-ietf-mmusic-rfc2326bis-04

draft-ietf-mmusic-rfc2326bis-03

draft-ietf-mmusic-rfc2326bis-02

draft-ietf-mmusic-rfc2326bis-01
