...
flowchart TD
    ...
    %% STREAMING LAYER
    SL["STREAMING LAYER"]
    HLS[["HLS Manifest Generator<br/>(gohlslib)"]]
    Trans[["Transcoding Engine<br/>(FFmpeg)"]]
    ...
    %% Invisible links for horizontal layout
    Web ~~~ Mobile ~~~ TV
    Skip ~~~ Trick ~~~ Chap
    Sync ~~~ Cast ~~~ Subs
    Media ~~~ MetaDB ~~~ Cache
    ...
    %% Layer styling
    style CL fill:#1976D2,stroke:#fff,stroke-width:2px,color:#fff
    style SL fill:#388E3C,stroke:#fff,stroke-width:2px,color:#fff
    style FL fill:#7B1FA2,stroke:#fff,stroke-width:2px,color:#fff
    style STL fill:#F57C00,stroke:#fff,stroke-width:2px,color:#fff
```
## Implementation
